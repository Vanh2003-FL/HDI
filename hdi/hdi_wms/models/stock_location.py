...
class StockLocation(models.Model):
    """
    ✅ INHERIT stock.location (Odoo core)
    
    KHÔNG thay đổi cấu trúc vị trí của Odoo
    CHỈ thêm:
    - Tọa độ X-Y-Z (warehouse coordinates)
    - Moving class (ABC classification)
    - Volume, capacity
    - Priority
    - WMS attributes
    
    ✅ Logic hierarchy, quant vẫn 100% core
    """
    ...
    def _compute_current_capacity(self):
        """
        Calculate current used capacity from batches
        ✅ Data từ hdi.batch nhưng batch sync với stock.quant (core)
        """
        for location in self:
    ...
    def get_available_capacity_for_product(self, product, quantity):
        """
        Check if location can accommodate product quantity
        Used by putaway suggestion engine
        """
        self.ensure_one()
    ...
    def _check_coordinates_unique(self):
        """Ensure coordinates are unique within warehouse"""
        for location in self:
            if location.coordinate_x or location.coordinate_y or location.coordinate_z:
