...
class HrDepartment(models.Model):
    ...
    def write(self, values):
        ...
        if any(key in values for key in ['name', 'active', 'block_id']):
    ...
    def _create_unit_hierarchy(self):
        vals = {
            ...
            "parent_id": self.env["ngsc.unit.hierarchy"].sudo().search([('block_id', '=', self.block_id.id)], limit=1).id
        }
        self.env["ngsc.unit.hierarchy"].sudo().create(vals)

    def _delete_unit_hierarchy(self):
        record = self.env["ngsc.unit.hierarchy"].sudo().search([('department_id', '=', self.id)], limit=1)
