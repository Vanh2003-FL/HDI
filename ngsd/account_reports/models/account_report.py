...
class AccountReport(models.AbstractModel):
        ...
        * The period type (e.g. quarter) if not specified explicitly.
        ...
        if not string:
            ...
            if mode == 'single':
                ...
                string = format_date(self.env, fields.Date.to_string(date_to), date_format='MMMM yyyy')
                ...
                dt_from_str = format_date(self.env, fields.Date.to_string(date_from), date_format='dd MMMM yyyy')
                dt_to_str = format_date(self.env, fields.Date.to_string(date_to), date_format='dd MMMM yyyy')
                string = _('Từ %s<br/>đến  %s') % (dt_from_str, dt_to_str)
    ...
    def _init_filter_analytic(self, options, previous_options=None):
        ...
        enable_analytic_tags = self.env.user.has_group('analytic.group_analytic_tags')
    ...
    def _init_filter_department(self, options, previous_options=None):
        ...
        selected_departments = selected_department_ids and self.env['hr.department'].browse(selected_department_ids) or \
    ...
    def _build_line_id(self, current):
        ...
        return '|'.join(
            '%s-%s-%s' % (markup, convert_none(model), convert_none(value)) for markup, model, value in current)
    ...
    def tax_tag_template_open_aml(self, options, params=None):
        ...
        domain = [('tax_tag_ids', 'in', tag_template.tag_ids.ids)] + self.env[
            'account.move.line']._get_tax_exigible_domain()
        return self.open_action(options, domain)
    ...
    def _set_context(self, options):
        ...
        if options.get('department_ids'):
            ctx['department_ids'] = self.env['hr.department'].browse(
                [int(partner) for partner in options['department_ids']]).ids
        if options.get('partner_categories'):
    ...
    def _get_html_render_values(self, options):
        return {
            'report': {
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the line_template
        ...
        render_values = self._get_html_render_values(options)
    ...
    def print_xlsx(self, options, ctx=False):
        return {
            'type': 'ir_actions_account_report_download',
            'data': {'model': self.env.context.get('model'),
                     'options': json.dumps(options),
    ...
    def get_xlsx(self, options, response=None):
        ...
        date_default_col1_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1, 'num_format': 'dd-mm-yyyy'})
        date_default_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'num_format': 'dd-mm-yyyy', 'border': 1})
        default_col1_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        default_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'font_size': 12, 'border': 1})
        title_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'bold': True, 'border': 1})
        level_0_style = workbook.add_format({'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_1_style = workbook.add_format({'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_2_col1_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_2_col1_total_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_2_style = workbook.add_format({'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_3_col1_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_3_col1_total_style = workbook.add_format(
            {'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        level_3_style = workbook.add_format({'valign': 'vcenter', 'text_wrap': True, 'font_name': 'Arial', 'border': 1})
        ...
        for header in headers:
            ...
            for column in header:
                column_name_formated = html2plaintext(
                    column.get('name', '').replace('<br/>', ' ').replace('&nbsp;', ' '))
                ...
                if colspan == 1 and rowspan == 1:
                    ...
                    sheet.merge_range(y_offset, x_offset, y_offset + rowspan - 1, x_offset + colspan - 1,
                                      column_name_formated, title_style)
                x_offset += colspan
            y_offset += 1
        ...
        for header in headers:
            ...
            for column in header:
                ...
                col_text = html2plaintext(name.replace('<br/>', ' ').replace('&nbsp;', ' '))
                width = len(col_text) + 2
                column_widths[col_idx] = max(column_widths.get(col_idx, 0), width)
                col_idx += column.get('colspan', 1)
            ...
            for line in lines:
                ...
                column_widths[0] = max(column_widths.get(0, 0), len(text) + 2)

                # Các cột còn lại
                for i, cell in enumerate(line.get('columns', []), start=1):
                    cell_type, cell_value = self._get_cell_type_value(cell)
                    text = str(cell_value) if cell_value is not None else ''
                    column_widths[i] = max(column_widths.get(i, 0), len(text) + 2)
    ...
    def _get_cell_type_value(self, cell):
        try:
            if isinstance(float(html2plaintext(cell['name'])), (float, int)):
                return ('number', float(html2plaintext(cell['name'])))
        except:
            pass
        try:
            if html2plaintext(cell['name']).endswith("%") and isinstance(
                    float(html2plaintext(cell['name']).split('%')[0]), (float, int)):
                return ('number', float(html2plaintext(cell['name']).split('%')[0]) / 100)
        except:
            pass
        if 'date' not in cell.get('class', '') or not html2plaintext(cell.get('name')):
            # cell is not a date
            return ('text', html2plaintext(cell.get('name', '')))
        if isinstance(html2plaintext(cell['name']), (float, datetime.date, datetime.datetime)):
            # the date is xlsx compatible
            return ('date', html2plaintext(cell['name']))
        try:
            # the date is parsable to a xlsx compatible date
            lg = self.env['res.lang']._lang_get(self.env.user.lang) or get_lang(self.env)
            return ('date', datetime.datetime.strptime(html2plaintext(cell['name']), lg.date_format))
        except:
            # the date is not parsable thus is returned as text
            return ('text', html2plaintext(cell['name']))
    ...
    def get_vat_for_export(self, options):
        ...
        if options['fiscal_position'] in {'all', 'domestic'}:
            ...
            if not company.vat:
                ...
                raise RedirectWarning(_('No VAT number associated with your company. Please define one.'), action.id,
                                      _("Company Settings"))
            return company.vat
        else:
