...
class ResourceAccountReportWizard(models.TransientModel):
    _name = "resource.account.report.wizard"
    _description = "Thông tin nguồn lực"
    ...
    resource_planing_id = fields.Many2one("en.resource.planning")
    ...
    is_hidden = fields.Boolean('Hiển thị nguồn lực ẩn')
    ...
    def do(self):
        ...
        action['target'] = 'main'
        action['context'] = {'model': 'resource.account.report',
                             'employee_id': self.employee_id.id,
                             'unit': self.unit, 
                             'employee_ids': self.employee_ids.ids,
                             'department_ids': self.department_ids.ids,
                             'is_hidden': self.is_hidden,
                             'resource_planing_id' : self.resource_planing_id.id}
        return action


class ResourceAccountReport(models.AbstractModel):
    _name = "resource.account.report"
    _description = "Thông tin nguồn lực"
    ...
    def _get_columns(self, options):
        ...
        for date_step in date_utils.date_range(datetime_from, datetime_to, relativedelta(months=1)):
            x += 1
            columns_monthly += [{'pre-offset': z + 1, 'name': f'{date_step.strftime("%m/%Y")}', 'colspan': 4, 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'}]
            columns_details += [
                {'pre-offset': z + 1, 'name': 'Thời gian', 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
                {'pre-offset': z + 2, 'name': 'Mã dự án', 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
                {'pre-offset': z + 3, 'name': 'Workload', 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
                {'pre-offset': z + 4, 'name': f'{unit}', 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
            ]
            z += 4
        for date_step in date_utils.date_range(datetime_from, datetime_to, relativedelta(months=1)):
            x += 1
        ...
        columns_names = [
            {'name': 'Mã nhân sự', 'rowspan': 3, 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
            ...
            {'name': 'Nguồn lực kế hoạch sử dụng trong dự án', 'colspan': int(x + (x*1/3)), 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
            {'name': 'Nguồn lực kế hoạch sử dụng trong Opp', 'colspan': int(x + (x*1/3)), 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'},
            {'name': 'Nguồn lực còn lại mà dự án có thể sử dụng', 'colspan': int(x / 3), 'style': 'background-color:#3F4C6A;color:white;text-align:left; white-space:nowrap;'}
        ]
    ...
    def _set_context(self, options):
        ...
        if not unit:
            wizard = self.env['resource.account.report.wizard'].search([('create_uid', '=', self.env.user.id)], order='id desc', limit=1)
            unit = wizard.unit
    ...
    def _get_options(self, previous_options=None):
        ...
        lst_key = ['employee_id', 'unit', 'resource_planing_id', 'employee_ids', 'department_ids', 'is_hidden']
        for k in lst_key:
    ...
    def _get_lines(self, options, line_id=None):
        ...
        # pass resource_planning_id via context
        # but if we reload the page, no resource_planing_id is provided so the else statement will be used. Maybe we could fix it later
        #mnd
        plan_id = options.get("resource_planing_id", False)
        employee_ids = options.get('employee_ids')
        department_ids = options.get('department_ids')
        is_hidden = options.get('is_hidden')
        domain_inactive = ['|', ('en_status', '!=', 'inactive'), '&', ('en_status', '=', 'inactive'), ('departure_date', '>', date_from)]
        if plan_id:
            resource_line = self.env["en.resource.planning"].browse(plan_id)
            employees = resource_line.order_line.employee_id

        elif employee_ids or department_ids or is_hidden:
            domain = []
            if employee_ids:
            ...
            domain = [('is_hidden', '=', False), ('en_status', 'in', ['active', 'semi-inactive', 'inactive'])]
            domain += domain_inactive
            employees = self.env['hr.employee'].with_context(active_test=False).search(domain)
        background = '#FFFFFF'
        leftover_tt = 0
        with localcontext() as ctx:
            ...
            print(5212 in employees.ids)
            for employee in employees:
                tech_data = self.env['en.technical.model'].convert_daterange_to_data(employee, datetime_from.date(), datetime_to.date())
                detail_data = self.env['en.resource.detail'].convert_daterange_to_data(employee, datetime_from.date(), datetime_to.date())
                ...
                for date_step in date_utils.date_range(datetime_from, datetime_to, relativedelta(months=1)):
                    ...
                    for resource in self.env['en.resource.detail'].search([('order_id.active', '=', True), ('employee_id', '=', employee.id), ('order_id.state', 'in', state_resource), '|',
                        ...
                        #     tech = tech_data.get(d)
                        #     if tech and tech.get('tech_type') not in ['off', 'holiday', 'not_work', 'layoff']:
                        #         resource_leftover = resource.workload * 8
                        #         if unit == 'MM':
                        #             value += resource_leftover / total_left if total_left else 0
                        #         elif unit == 'MD':
                        #             value += resource_leftover / 8
                        #         else:
                        #             value += resource_leftover
                        # value = Decimal(value * 1000).to_integral_value(rounding=ROUND_HALF_UP)
                        mm_rate = resource.order_id.mm_rate or 0.0
                    ...
                    if leftover:
                        ...
                        print('%s\t%s\t%s'%(employee.id, leftover_tt, leftover))
                    # leftover = Decimal(max([leftover * 1000, 0])).to_integral_value(rounding=ROUND_HALF_UP)
                    leftover = Decimal(max([leftover * 1000, 0])).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)
                    columns += [
                        {'name': '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                        {'name': '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                        {'name': '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                        {'name': f'{Decimal(total_value).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)}' if total_value else '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                    ]
                    plus_columns += [
                        {'name': '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                        {'name': '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                        {'name': '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                        {'name': f'{Decimal(plus_total_value).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)}' if plus_total_value else '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'},
                    ]
                    extra_columns += [{'name': f'{Decimal(leftover/1000).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)}' if leftover else '', 'style': f'background-color:{background};vertical-align:middle;text-align:right; white-space:nowrap;border:1px solid #000000'}]
                    ...
                    if dated:
                        ...
                        min_dated = dated[0]
                        max_dated = dated[0]
                        current_wl = 1 - detail_data.get(dated[0])
                        value = 0
                        for d in dated:
                            wl = 1 - detail_data.get(d)
                            ...
                            if min_dated == max_dated:
                                dated_txt += [f'<div style="white-space:nowrap;">{max_dated.strftime(lg.date_format)} Workload: {current_wl / 10}%</div>']
                                current_wl = wl
                            else:
                                dated_txt += [f'<div style="white-space:nowrap;">{min_dated.strftime(lg.date_format)} → {max_dated.strftime(lg.date_format)} Workload: {current_wl / 10}%</div>']
                                current_wl = wl
                            min_dated = d
                            max_dated = d
                        else:
                ...
                if is_child:
                    for i in range(max_child_number):
                        ...
                        for project in project_child_columns:
                            new_child_columns += [{'name': x, 'style': f'background-color:{background};vertical-align:middle;text-align:left; white-space:nowrap;border:1px solid #000000'} for x in (project[i] if len(project) > i else ['', '', '', ''])]
                        new_child_columns += [{'name': z, 'style': f'background-color:{background};vertical-align:middle;text-align:left; white-space:nowrap;border:1px solid #000000'} for opp in opp_child_columns for z in (opp[i] if len(opp) > i else ['', '', '', ''])]
                        new_child_columns += [{'name': d[i] if len(d) > i else '', 'style': f'background-color:{background};vertical-align:middle;text-align:left; white-space:nowrap;border:1px solid #000000'} for d in extra_child_columns]
