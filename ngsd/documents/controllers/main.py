...
class ShareRoute(http.Controller):
    ...
    @http.route(['/document/folder/zip'], type='http', auth='user')
    def get_folder_zip(self, folder_id, zip_name, **kw):
        ...
        """
        env = request.env
        folder = env['documents.folder'].browse(int(folder_id))
        if not folder.exists() or not folder.can_download:
            return request.not_found()
        response = self._make_zip(zip_name, env['documents.document'].search([('folder_id', 'child_of', int(folder_id))]))
        return response
    ...
    def get_zip(self, file_ids, zip_name, **kw):
        ...
        env = request.env
        files = env['documents.document'].browse(ids_list)
        if any(not file.exists() or not file.can_download for file in files):
            return request.not_found()
...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        user_view_document_button = self.env.user.has_group('documents.group_documents_technical,ngsd_base.group_pm,ngsd_base.group_tvda')
        result['user_view_document_button'] = user_view_document_button
        return result
