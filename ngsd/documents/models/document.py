...
class Document(models.Model):
    ...
    # Attachment
    en_project_id = fields.Many2one(string='Dự án', comodel_name='project.project', related='folder_id.en_project_id')
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name == 'folder_id':
            ...
            non_project = self._context.get('non_project', False)
            is_project = self._context.get('is_project', False)
            fields = ['display_name', 'description', 'parent_folder_id']
            if self._context.get('default_en_project_id'):
                available_folders = self.env['documents.folder'].search([('en_project_id', '=', self._context.get('default_en_project_id'))])
            else:
