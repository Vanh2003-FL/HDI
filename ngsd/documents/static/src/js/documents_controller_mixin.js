...
const DocumentsControllerMixin = Object.assign({}, fileUploadMixin, {
    ...
    events: {
        ...
        'click .o_documents_kanban_new_folder': '_onClickDocumentsNewFolder',
    ...
    custom_events: Object.assign({}, fileUploadMixin.custom_events, {
        ...
        folder_download: '_onFolderDownload',
    ...
    updateButtons() {
        ...
        if (!session?.user_view_document_button) {
            this.$buttons.find('.o_documents_kanban_upload').hide();
            this.$buttons.find('.o_documents_kanban_new_folder').hide();
            this.$buttons.find('.o_documents_kanban_url').hide();
        }
     ...
     * @param {MouseEvent} ev
     */
    _onClickDocumentsNewFolder(ev) {
        ev.preventDefault();
        const context = this.model.get(this.handle, {raw: true}).getContext();
        this.do_action('documents.folder_form_popup_action', {
            additional_context: {
                unite:true,
                default_parent_folder_id: this.searchModel.get('selectedFolderId'),
            },
            fullscreen: config.device.isMobile,
            on_close: () => {
     ...
     * @param {integer[]} ev.data.resIds
     */
    _onFolderDownload(ev) {
        ev.stopPropagation();
        const folderID = ev.data.folderID;
        session.get_file({
            url: '/document/folder/zip',
            data: {
                folder_id: folderID,
                zip_name: `documents-${moment().format('YYYY-MM-DD')}.zip`
            },
