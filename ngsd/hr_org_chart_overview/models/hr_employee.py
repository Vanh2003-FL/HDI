...
class HrEmployee(models.Model):
    ...
    def _get_employee_domain(self, parent_id):
        ...
        if self._context.get('en_department_ids'):
            domain += [('department_id', 'in', self._context.get('en_department_ids'))]
        if self._context.get('en_block_ids'):
            domain += [('en_block_id', 'in', self._context.get('en_block_ids'))]
        if self._context.get('en_area_ids'):
            domain += [('en_area_id', 'in', self._context.get('en_area_ids'))]
        return domain
    ...
    def _get_employee_data(self, level=0):
        ...
        # image = f'/web/image?model=hr.employee&id={self.id}&field=avatar_128'
        if not image:
