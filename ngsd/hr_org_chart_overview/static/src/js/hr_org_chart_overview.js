odoo.define("hr_org_chart_overview", function (require) {
    ...
    var HrOrgChartOverview = AbstractAction.extend({
        ...
        init: function (parent) {
            ...
            this.model_name = parent?.props.widgetArgs[0]?.params?.model || 'hr.employee'
            this.can_click_node = parent?.props.widgetArgs[0]?.params?.can_click_node
        ...
        willStart: function () {
            ...
            var def = this._rpc({
                ...
                context: self.searchModelConfig.context
            }).then(function (res) {
        ...
        _getNodeTemplate: function (data) {
            var image = '';
            if (data.type_image === 'base64'){
                image = `<span class="image"><img src="data:image/png;base64,${data.image}"/></span>`
            }
            if (data.type_image === 'url'){
                image = `<span class="image"><img src="${data.image}"/></span>`
            }
            return `
                ${image}
        ...
        _renderBreadcrumb: function () {
            ...
            this.$breadcrumb.prepend(`
                <li class="breadcrumb-item active">${this._title || 'Organizational Chart'}</li>
            `);
        },
        ...
        start: function () {
            var oc = this.$("#chart-container").orgchart({
