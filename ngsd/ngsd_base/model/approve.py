...
class ApproveFlow(models.Model):
    ...
    model_id = fields.Many2one(comodel_name="ir.model", string="Loại chứng từ", required=True, ondelete='cascade', domain="[('field_id.name','=','en_approve_line_ids'), ('model', '!=', 'ngsd.approval')]")
    model = fields.Char(related='model_id.model')
    project_ids = fields.Many2many(comodel_name="project.project", string="Dự án áp dụng")
    domain = fields.Char(string="Điều kiện")
    rule_ids = fields.One2many(comodel_name="en.approve.rule", inverse_name="flow_id", string="Quy tắc phê duyệt")
    ...
    def _normalize_visible_sequence(self, rule_ids_data, existing_rules):
        ...
        sequence_mapping = {old_seq: idx + 1 for idx, old_seq in enumerate(unique_sequences)}
        # Cập nhật visible_sequence trong rule_ids_data
        for rule_data in rule_ids_data:
            rule_data['visible_sequence'] = sequence_mapping.get(rule_data.get('visible_sequence', 0), 1)
    ...
    def create(self, vals):
        ...
        if 'rule_ids' in vals:
            ...
            if rule_ids_data:
                ...
                normalized_rules = self._normalize_visible_sequence(rule_ids_data, existing_rules=self.env['en.approve.rule'])
                rule_idx = 0
                new_rule_ids = []
        ...
        return super(ApproveFlow, self).create(vals)
    ...
    def write(self, vals):
        ...
        if 'rule_ids' in vals:
            ...
            if rule_ids_data:
                # Chuẩn hóa visible_sequence dựa trên existing_rules và rule_ids_data
                normalized_rules = self._normalize_visible_sequence(rule_ids_data, existing_rules=existing_rules)
                rule_idx = 0
                new_rule_ids = []
                for cmd, updated_rule in zip(vals['rule_ids'], updated_rule_ids):
                    if cmd[0] == 0:
                        ...
                        new_rule_ids.append((1, cmd[1], {'visible_sequence': normalized_rules[rule_idx]['visible_sequence']}))
                        rule_idx += 1
                    else:
                        new_rule_ids.append(cmd)
                vals['rule_ids'] = new_rule_ids
...
class ApproveRule(models.Model):
    ...
    flow_id = fields.Many2one(comodel_name="en.approve.flow", string="Vai trò", required=True, ondelete='cascade')
    sequence = fields.Integer(help="Gives the sequence of this line when displaying.", default=9999)

    visible_sequence = fields.Integer("Trình tự", help="Displays the sequence of the line.")
    ...
    resource_unit = fields.Selection(string='Đơn vị so sánh', selection=[('mm', 'MM'), ('budget', 'Budget ban đầu'), ('baseline', 'Baseline gần nhất')], default='mm')
...
class ApproveLine(models.Model):
    ...
    approver_user_ids = fields.Many2many(comodel_name="res.users", string="Người có thể duyệt")
    user_id = fields.Many2one(comodel_name="res.users", string="Người đã duyệt")
    date = fields.Datetime(string="Thời gian duyệt")
    state = fields.Selection([('sent', "Chờ duyệt"), ('approved', "Đã duyệt"), ('refused', "Từ chối"), ], string="Trạng thái", default='sent', required=True, readonly=True, copy=False)
...
class NgsdApproval(models.AbstractModel):
    ...
    def unlink(self):
        ...
        for rec in self:
            to_approver_remove |= self.env['en.current.approver'].search([('res_model', '=', rec._name), ('res_id', '=', rec.id)])
            to_line_remove |= self.env['en.approve.line'].search([('res_model', '=', rec._name), ('res_id', '=', rec.id)])
        res = super().unlink()
    ...
    def button_sent(self):
        ...
        if not en_approve_line_ids: raise exceptions.ValidationError('Không tìm thấy quy trình duyệt tương ứng với chứng từ')
    ...
    def button_sent_1(self):
        ...
        en_approve_line_ids, en_next_approver_ids = self.get_en_approve_line_ids()
        if not en_approve_line_ids: raise exceptions.ValidationError('Không tìm thấy quy trình duyệt tương ứng với chứng từ')
        self.with_context(clean_context(self._context)).write({'en_request_user_id': self.env.user.id, 'en_request_date': fields.Datetime.now(), 'state': self.sent_state(), 'en_next_approver_ids': [(6, 0, en_next_approver_ids.ids)], 'en_approve_line_ids': [(5, 0, 0)] + en_approve_line_ids})
        self.state_notify(self.get_message())
        return True

    def state_notify(self, message=False):
        self.clear_caches()
        for approval in self.en_approve_line_ids.search([('res_id', 'in', self.ids), ('res_model', '=', self._name)]):
    ...
    def state_change(self, approve_state):
        ...
        update_value = dict(state=approve_state, date=fields.Datetime.now(), user_id=user.id)
        en_approve_line_ids = []
        ...
        if approve_state == 'refused':
            ...
            if not all_approved and next_sequence:
                ...
                next_approvals = approvals.filtered(
                    lambda a: a.sequence == next_sequence and a.state == 'sent' and a.id != updated_approval_id
                )
    ...
    def get_message(self):
        return 'Bạn có bản ghi %s cần duyệt: %s' % (self.env['ir.model']._get(self._name).display_name, self.display_name)
    ...
    def get_en_approve_line_ids(self):
        ...
        self.clear_caches()
        self = self.sudo()
        processes = self.env['en.approve.flow'].search(self.get_flow_domain(), order='id desc')
    ...
    en_approve_line_ids = fields.One2many(string='Thông tin phê duyệt', comodel_name='en.approve.line', inverse_name='res_id', domain=lambda self: [('res_model', '=', self._name)], readonly=True)
    en_request_user_id = fields.Many2one(string='Người gửi duyệt', comodel_name='res.users', copy=False, readonly=True)
    en_request_date = fields.Datetime(string='Ngày gửi duyệt', copy=False, readonly=True)
    en_next_approver_ids = fields.Many2many(string='Người duyệt tiếp theo', comodel_name='res.users', compute='_compute_en_next_approver_ids', inverse='_inverse_en_next_approver_ids', search='_search_en_next_approver_ids', readonly=True)

    def _compute_en_next_approver_ids(self):
        for rec in self:
            rec.en_next_approver_ids = self.env['en.current.approver'].search([('res_id', '=', rec._origin.id), ('res_model', '=', rec._name)]).mapped('user_id')

    last_approver_id = fields.Many2one(string='Người duyệt gần nhất', comodel_name='res.users', compute='_compute_last_approver')
    last_approver_date = fields.Datetime(string='Thời gian duyệt gần nhất', compute='_compute_last_approver')

    @api.depends('en_approve_line_ids')
    def _compute_last_approver(self):
        for rec in self:
            last_line = rec.en_approve_line_ids.filtered(lambda l: l.state == 'approved')[-1:]
    ...
    def _search_en_next_approver_ids(self, operator, operand):
        return [('en_last_approver_ids', 'in', self.env['en.current.approver'].sudo()._search([('user_id', operator, operand)]))]
    ...
    en_last_approver_ids = fields.One2many(string='Người duyệt gần nhất', comodel_name='en.current.approver', inverse_name='res_id', domain=lambda self: [('res_model', '=', self._name)])
...
class CurrentApprover(models.Model):
    ...
    res_id = fields.Integer(string='ID tài nguyên')
    res_model = fields.Char(string='Model tài nguyên')
    user_id = fields.Many2one(string='Người duyệt tiếp theo', comodel_name='res.users')
    ...
    def do(self):
        ...
        for record in records:
            ...
            if record.state_change('refused'):
                ...
                record.sudo().message_post(body=f'{self.env.user.name} đã từ chối vào {fields.Datetime.now().strftime("%d-%m-%Y %H:%M:%S")}! Lý do từ chối: {self.name}')
        ...
        return {
            'effect': {
                'fadeout': 'slow',
                'message': 'Đã từ chối!',
                'img_url': '/web/image/%s/%s/image_1024' % (self.env.user._name, self.env.user.id) if self.env.user.image_1024 else '/web/static/src/img/smile.svg',
                'type': 'rainbow_man',
            }
        }
