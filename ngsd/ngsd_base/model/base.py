...
class Base(models.AbstractModel):
    ...
    def read(self, fields=None, load='_classic_read'):
        if fields == ['im_status'] or fields == ['commercial_partner_id'] or fields == ['display_name'] or fields == ['display_name', 'color']:
            self = self.sudo()
...
class MailThread(models.AbstractModel):
    ...
    def _message_compute_author(self, author_id=None, email_from=None, raise_exception=True):
        author_id, email_from = super(MailThread, self)._message_compute_author(author_id, email_from, raise_exception)
