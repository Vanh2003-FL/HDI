...
class EnBorrowEmployee(models.Model):
    ...
    borrower_id = fields.Many2one('res.users', 'Người đi mượn', default=lambda self: self.env.uid)
    department_borrower_id = fields.Many2one('hr.department', 'Trung tâm đi mượn', compute=False, store=True, readonly=False, domain="[('id', 'in', department_borrower_domain)]")
    department_borrower_domain = fields.Many2many('hr.department', 'Trung tâm đi mượn', compute='_get_department_borrower_domain')
    lender_id = fields.Many2one('hr.employee', 'Người cho mượn', related='department_lender_id.manager_id')
    department_lender_id = fields.Many2one('hr.department', string='Trung tâm cho mượn', required=True, domain="[('id', 'in', department_lender_ids)]")
    department_lender_ids = fields.Many2many('hr.department', string='Domain Trung tâm cho mượn', default=lambda self: self._compute_department_ids())
    ...
    date = fields.Date('Ngày mượn', default=fields.Date.today(), required=True, states=READONLY_STATES)
    code = fields.Char('Mã phiếu')
    borrow_employee_detail_ids = fields.One2many('en.borrow.employee.detail', 'borrow_employee_id', string='Chi tiết mượn nhân sự')
    count_bill_borrow = fields.Integer('Nhân sự đi mượn', compute='_compute_count_bill_borrow')
    lender_employee_ids = fields.One2many('en.lender.employee', 'borrow_employee_id', string='Danh sách mượn nhân sự')
    ...
    def _compute_department_ids(self):
        department_ids = self.env['hr.department'].search([('id', '!=', self.env.user.employee_id.department_id.id)]).ids
    ...
    def _get_department_borrower_domain(self):
        for rec in self:
            ...
            if rec.borrower_id.employee_id:
                department = self.env['hr.department'].search(['|', ('manager_id', '=', rec.borrower_id.employee_id.id), ('deputy_manager_id', '=', rec.borrower_id.employee_id.id)])
    ...
    def button_to_approve(self):
        ...
        if not rslt: return
        if self.approver_id: self.send_notify(f'Bạn có phiếu mượn {self.display_name} cần được duyệt', self.approver_id)
        self.write({'state': 'to_approve'})

    def new_lender_employee(self):
        return self.open_form_or_tree_view('ngsd_base.en_lender_employee_act', False, False, {'default_date': fields.Date.today(), 'default_borrow_employee_id': self.id}, 'Tạo danh sách', 'current')

    @api.model
    def create(self, values):
        values['code'] = self.env['ir.sequence'].next_by_code('code.borrow.employee')
        return super(EnBorrowEmployee, self).create(values)
...
class EnBorrowEmployeeDetail(models.Model):
    ...
    borrow_employee_id = fields.Many2one('en.borrow.employee', 'Mượn nhân sự', ondelete='cascade')
