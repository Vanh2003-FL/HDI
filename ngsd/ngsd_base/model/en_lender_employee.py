...
class EnLenderEmployee(models.Model):
  ...
  department_borrower_id = fields.Many2one('hr.department', 'Trung tâm đi mượn',
                                           related='borrow_employee_id.department_borrower_id',
                                           store=True)
  lender_id = fields.Many2one('hr.employee', 'Người cho mượn',
                              related='borrow_employee_id.lender_id')
  department_lender_id = fields.Many2one('hr.department',
                                         related='borrow_employee_id.department_lender_id',
  ...
  def to_done(self):
    ...
    if name_lender:
      self.send_notify(
        f'{name_lender} đã cho bạn mượn nhân sự. Bấm vào nút phía dưới để xem chi tiết.',
        self.borrower_id)
    else:
      self.send_notify(
        f'Người quản lý {self.department_lender_id.name} đã cho bạn mượn nhân sự',
        self.borrower_id)
    self.state = 'done'
    return True
    ...
    for rec in self:
      if rec.department_borrower_id and rec.department_lender_id:
        result.append((rec.id,
                       "%s mượn nhân sự %s" % (rec.department_borrower_id.name,
                                               rec.department_lender_id.name)))
      else:
    ...
    for values in vals_list:
      if 'code' not in values or not values['code']:
        values['code'] = self.env['ir.sequence'].next_by_code(
          'code.lender.employee')
    return super().create(vals_list)

  def unlink(self):
    if any(rec.state != 'pending' for rec in self):
      raise ValidationError(
        'Không được xóa các bản ghi ở trạng thái Đã bàn giao/Hủy')
...
class EnLenderEmployeeDetail(models.Model):
  ...
  lender_employee_id = fields.Many2one('en.lender.employee',
                                       'Danh sách mượn nhân sự',
                                       ondelete='cascade')
  add_workload = fields.Float('Gửi duyệt YC tăng wl', default=False, copy=False,
                              readonly=1)

  def action_send_mail_ahead_schedule(self):
    today = date.today()
    all_lender_employee_ids = self.search(
    ...
    for lender in all_lender_employee_ids:
      ...
      lender.lender_employee_id.send_notify(message,
  ...
  def action_return_on_time(self):
    ...
    for line in list_lender_employee:
      ...
      line.lender_employee_id.send_notify(message,
                                          line.lender_employee_id.lender_id.user_id | line.lender_employee_id.borrower_id)
      ...
      if line.date_return == today:
        resource_project = self.env['resource.project'].search(
            [('project_id.en_department_id', '=',
              self.lender_employee_id.department_borrower_id.id),
             ('employee_id', '=', line.employee_id.id),
             ('date_start', '>=', line.date_start),
             ('date_end', '<=', line.date_end), ('state', '=', 'active'),
             ('is_borrow', '=', True)])
        for resource in resource_project:
          resource.action_leave(date_end)
      resource_department = self.env['en.department.resource'].search(
  ...
  def action_returned(self, date_return):
    ...
    if resource_project:
      raise ValidationError(
        'Bạn phải cho nhân sự đó rời dự án trước khi trả người')
    message = f'{user_lender.name} đã trả nhân sự {self.employee_id.name} ngày {date_return.strftime("%d/%m/%Y")}'
    self.lender_employee_id.send_notify(message,
                                        self.lender_employee_id.lender_id.user_id)
  ...
  def button_receive(self):
    user_lender = self.env.user
    department_resource = self.env['en.department.resource'].search_count(
        [('employee_id', '=', self.employee_id.id),
         ('lender_employee_detail_id.state', '!=', 'no_receive'),
         ('borrow_department_id', '=',
          self.lender_employee_id.department_borrower_id.id),
         ('date_start', '<=', self.date_end),
         ('date_end', '>=', self.date_start)])
    if department_resource:
      raise ValidationError(
        f'Nhân sự {self.employee_id.display_name} đã được mượn trong quãng thời gian này.')
    if self.lender_employee_id.lender_id:
      self.lender_employee_id.send_notify(
        f'{user_lender.name} đã tiếp nhận {self.employee_id.name}',
        self.lender_employee_id.lender_id.user_id)
  ...
  def button_no_receive(self):
    ...
    if self.lender_employee_id.lender_id:
      self.lender_employee_id.send_notify(
        f'{user_lender.name} không tiếp nhận {self.employee_id.name}',
        self.lender_employee_id.lender_id.user_id)
  ...
  @api.depends('lender_employee_id',
               'lender_employee_id.borrow_employee_id.borrow_employee_detail_ids')
  ...
  def _constrains_date_no_duplicate(self):
    for rec in self:
      count_duplicate = self.search_count([
        ...
        ('id', '!=', rec.id),
        ('lender_employee_id.department_borrower_id', '=',
         rec.lender_employee_id.department_borrower_id.id),
        ('date_start', '<=', rec.date_end), ('date_end', '>=', rec.date_start),
        ('state', '!=', 'no_receive')
      ])
      if count_duplicate:
        raise ValidationError(
          f'Nhân sự {rec.employee_id.display_name} đã được cho {rec.lender_employee_id.department_borrower_id.display_name} mượn trong khoảng thời gian này. Vui lòng chọn lại thời gian hoặc nhân sự.')

  show_add_workload_request = fields.Boolean(
    compute='_compute_show_add_workload')
  show_add_workload_confirm = fields.Boolean(
    compute='_compute_show_add_workload')
