...
class QualityControl(models.Model):
    ...
    seq_id = fields.Integer(string='💰', default=lambda self: int(self.env['ir.sequence'].next_by_code('quality.seq.id')), copy=False)
    ...
    @api.depends('project_id', 'project_id.en_quality_control_ids', 'parent_id', 'parent_id.child_ids', 'seq_id', 'version_type', 'state')
    def _compute_version_number(self):
        ...
        for project in self.filtered(lambda x: not x.parent_id and x.version_type == 'plan').mapped("project_id"):
            sequence = 1
            wbs = project.en_quality_control_ids.filtered(lambda x: not x.parent_id and x.version_type == 'plan')
            for line in sorted(wbs, key=lambda l: l.seq_id):
        ...
        for project in self.filtered(lambda x: not x.parent_id and not x.version_type == 'plan').mapped("project_id"):
            sequence = 1
            wbs = project.en_quality_control_ids.filtered(lambda x: not x.parent_id and not x.version_type == 'plan')
            for line in sorted(wbs, key=lambda l: l.seq_id):
                line.version_number = f"{sequence}.0"
                sequence += 1

    parent_id = fields.Many2one(string='Thuộc về baseline', comodel_name='en.quality.control', compute_sudo=True, compute='_compute_parent_id', store=True)
    ...
    def _compute_parent_id(self):
        for rec in self:
            ...
            if rec.version_type == 'plan':
                parent_id = self.env['en.quality.control'].search([('version_type', '=', 'baseline'), ('project_id', '=', rec.project_id.id), ('state', 'in', ['approved', 'expire']), ('id', '<', rec._origin.id)], limit=1, order='technical_field_before desc')
            rec.parent_id = parent_id

    child_ids = fields.One2many(string='Plan', comodel_name='en.quality.control', inverse_name='parent_id')
    ...
    def _constrains_no_more_than_one(self):
        if self._context.get('import_file') and any(rec.sudo().search_count([('project_id', '=', rec.project_id.id), ('state', 'in', ['draft', 'to_approve'])]) > 1 for rec in self):
            raise ValidationError('Hiện tại đang có bản ghi KHNL chưa được duyệt, vui lòng duyệt để tạo bản ghi KHNL mới')
    ...
    def _constrains_project_id_state(self):
        if self._context.get('import_file') and any(self.env['en.quality.control'].search_count([('project_id', '=', rec.project_id.id), ('state', '=', 'draft')]) > 1 for rec in self):
            raise ValidationError(f'Bạn không thể tạo hai KHNL ở trạng thái dự kiến ')

    def action_open_new_tab(self):
        return self.open_form_or_tree_view('ngsd_base.quality_control_act', False, self, {'default_project_id': self.id})
    ...
    def _compute_technical_field_27766(self):
        for rec in self:
            ...
            if rec.state not in ['approved', 'refused'] or not rec.project_id:
                ...
                continue
            if rec._origin.id == self.env['en.quality.control'].search([('project_id', '=', rec.project_id.id), ('state', '=', 'approved')], order='id desc', limit=1).id:
    ...
    def unlink(self):
        for rec in self:
            if rec.state in READONLY_STATES.keys():
                raise UserError(f"Bản ghi đang ở trạng thái {dict(rec.fields_get(['state'])['state']['selection'])[rec.state]} . Không thể xóa bản ghi này !")
        return super().unlink()
    ...
    def _constrains_overload(self):
        ...
        for rec in self:
            ...
            for employee in set(rec.mapped('order_line.employee_id')):
                ...
                for line in rec.order_line.filtered(lambda x: x.employee_id == employee):
                    ...
                    for date_step in date_utils.date_range(datetime_start, datetime_end, relativedelta(days=1)):
                        if round(sum(self.env['en.quality.detail'].search([('order_id', '=', rec.id), ('employee_id', '=', employee.id), '&', ('date_start', '<=', date_step.date()), ('date_end', '>=', date_step.date())]).mapped('workload')) + sum(self.env['en.quality.detail'].search([('order_id.state', '=', 'approved'), ('order_id.project_id', '!=', rec.project_id.id), ('employee_id', '=', employee.id), '&', ('date_start', '<=', date_step.date()), ('date_end', '>=', date_step.date())]).mapped('workload')), 10) <= 1.2: continue
    ...
    def button_approved(self):
        ...
        if rslt:
            self.write({'seq_id': int(self.env['ir.sequence'].next_by_code('quality.seq.id'))})
    ...
    order_line = fields.One2many(string='Chi tiết kiểm soát chất lượng', states=EDIT_DRAFT_STATES, comodel_name='en.quality.detail', inverse_name='order_id', copy=False)

    mm_qa_project = fields.Float('MM dự án phân bổ cho QA', compute='_compute_mm_qa_project', store=True, compute_sudo=True)
    total_md = fields.Float('Tổng MD', compute='_compute_total_md', store=True, compute_sudo=True)
    total_mm = fields.Float('Tổng MM', compute='_compute_total_md', store=True, compute_sudo=True)
...
class QualityDetail(models.Model):
    ...
    old_line_id = fields.Many2one('en.quality.detail', string='Line cũ', readonly=True, copy=False)

    en_user_id = fields.Many2one('res.users', string='User của nhân viên', default=lambda self: self.env.user)
    order_id = fields.Many2one(string='Kiểm soát chất lượng', comodel_name='en.quality.control', required=True, ondelete='cascade', index=True, auto_join=True)
    employee_id = fields.Many2one(string='Tên nhân sự', comodel_name='hr.employee', default=lambda self: self.env.user.employee_id.id, index=True)
            ...
            if rec.old_line_id or rec.order_id.parent_id:
                if date_today <= 5:
                    if rec.date_start and rec.date_start < month_pre:
                        raise ValidationError(f'Giá trị ngày bắt đầu phải lớn hơn {month_pre.strftime("%d/%m/%Y")}')
                else:
                    if rec.date_start and rec.date_start < month_current:
                        raise ValidationError(f'Giá trị ngày bắt đầu phải lớn hơn {month_current.strftime("%d/%m/%Y")}')
            ...
            if rec.old_line_id or rec.order_id.parent_id:
                if date_today <= 5:
                    if rec.date_end and rec.date_end < month_pre:
                        raise ValidationError(f'Giá trị ngày kết thúc phải lớn hơn {month_pre.strftime("%d/%m/%Y")}')
                else:
                    if rec.date_end and rec.date_end < month_current:
                        raise ValidationError(f'Giá trị ngày kết thúc phải lớn hơn {month_current.strftime("%d/%m/%Y")}')
    ...
    def _constrains_date_start(self):
        for rec in self:
            expt_txt = ''
            for resource in rec.order_id.project_id.en_resource_id.order_line.filtered(lambda x: x.employee_id == rec.employee_id):
                if (rec.date_start and rec.date_start < resource.date_start) or (rec.date_end and rec.date_end > resource.date_end):
    ...
    def create(self, values):
        ...
        return super(QualityDetail, self).create(values)
