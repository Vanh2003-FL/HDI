...
class EmployeeCategory(models.Model):
    ...
    def cron_auto_apply(self):
        ...
        for rec in self.search([], order='day_auto_apply asc'):
            ...
            employees = self.env['hr.employee'].search([('official_working_day', '<=', start_date)])
            # employees.write({'category_ids': [(4, rec.id)]})
            employees.write({'category_ids': [(6, 0, rec.ids)]})
            employees_remove = self.env['hr.employee'].search([('active', '=', False), ('category_ids', '=', rec.id)])
...
class HrDepartment(models.Model):
    ...
    deputy_manager_id = fields.Many2one(string='Phó GĐ Khối NDU', comodel_name='hr.employee')
    en_project_manager_id = fields.Many2one(string='Giám đốc dự án', comodel_name='res.users')
    name = fields.Char(string='Tên Trung tâm/Ban')
    code = fields.Char(string='Mã Trung tâm/Ban')
    hr_employee_ids = fields.One2many(comodel_name='hr.employee', string='Nhân viên', inverse_name='department_id')
    count_hr_employee_ids = fields.Integer(string='Số lượng nhân viên trong Trung tâm/Ban', compute='compute_count_hr_employee_ids')
    en_department_ids = fields.One2many(comodel_name='en.department', inverse_name='department_id', string='Phòng')
    bod = fields.Boolean(default=False, string='BOD')

    employee_borrow_ids = fields.One2many('en.department.resource', 'borrow_department_id', 'Nhân sự mượn', domain=[('state', '=', 'active')])
    employee_lender_ids = fields.One2many('en.department.resource', 'department_id', 'Nhân sự cho mượn', domain=[('state', '=', 'active')])
    employee_lender_report_ids = fields.One2many('en.department.resource', 'department_id', 'Nhân sự cho mượn',)
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        department = self._search(domain + args, limit=limit, access_rights_uid=name_get_uid)
        return department

    def unlink(self):
        list_en_department = [rec.name for rec in self if any(en_department.department_id for en_department in rec.en_department_ids)]
        list_project_name = [rec.name for rec in self if self.env['project.project'].search([('en_department_id', '=', rec.id)])]
        if list_en_department and list_project_name:
            raise UserError(f"Trung tâm/ban {','.join(list_en_department)} đang có Phòng gắn với nó, vui lòng kiểm tra lại!\nTrung tâm/ban {','.join(list_project_name)} đang có Dự án gắn với nó, vui lòng kiểm tra lại!")
        elif list_en_department:
            raise UserError(f"Trung tâm/ban {','.join(list_en_department)} đang có Phòng gắn với nó, vui lòng kiểm tra lại!")
        elif list_project_name:
            raise UserError(f"Trung tâm/ban {','.join(list_project_name)} đang có Dự án gắn với nó, vui lòng kiểm tra lại!")
        return super().unlink()
...
class CalendarLeaves(models.Model):
    ...
    date_from_convert = fields.Date(compute='_compute_convert_date_from', store=True)
    ...
    def write(self, vals):
        ...
        if date_from or date_to or 'calendar_id' in vals:
            ...
            if timesheets:
                timesheets.with_context(no_constrains=True).write({'global_leave_id': False, 'en_state': 'new'})
        ...
        if global_time_off_updated:
            ...
            global_time_offs_with_leave_timesheet.with_context(no_constrains=True).sudo()._timesheet_create_lines()
        return result
    ...
    def get_domain_tech(self, check_holiday=True):
        ...
        for rec in self:
            ...
            if rec.resource_id:
                domain.append(('employee_id', '=', rec.resource_id.user_id.employee_id.id))
    ...
    def recompute_technumber(self, date_list):
        ...
        for date_l in date_list:
            need_recomputes |= self.env['en.technical.model'].search([('date', '>=', date_l[0]), ('date', '<=', date_l[1])])
...
class TechnicalModel(models.Model):
    ...
    employee_id = fields.Many2one(string='🚑', comodel_name='hr.employee', index=True)
    date = fields.Date(string='🚑', index=True)
    tech = fields.Char(string='🚑', compute_sudo=True, compute='_compute_technumber', store=True)
    tech_type = fields.Char(string='🚑', compute_sudo=True, compute='_compute_technumber', help='Phân biệt xem nhân viên có làm việc hay không', store=True)
    number = fields.Float(string='🚑', compute_sudo=True, compute='_compute_technumber', store=True)

    @api.depends('employee_id', 'date', 'employee_id.tz', 'employee_id.en_date_start', 'employee_id.departure_date', 'employee_id.en_day_layoff_from', 'employee_id.en_day_layoff_to')
    def _compute_technumber(self):
        for rec in self:
            ...
            if not employee or not calendar or not rec.date:
                ...
                continue
            date = rec.date
            comparedtime_from = timezone(tz).localize(datetime.combine(date, time.min)).astimezone(timezone('UTC')).replace(tzinfo=None)
            comparedtime_to = timezone(tz).localize(datetime.combine(date, time.max)).astimezone(timezone('UTC')).replace(tzinfo=None)
            workhours = calendar.get_work_hours_count(comparedtime_from, comparedtime_to, compute_leaves=False)
            if not workhours:
    ...
    def convert_daterange_to_hours(self, employee, start_date, end_date, exclude_tech=None, exclude_tech_type=['off', 'not_work', 'layoff', 'holiday']):
        if start_date > end_date:
            return 0
        exclude_tech_domain = ''
        if exclude_tech:
            exclude_tech_domain = ' AND tech not in %s' % (str(exclude_tech).replace('[', '(').replace(']', ')'))
        exclude_tech_type_domain = ''
        if exclude_tech_type:
            exclude_tech_type_domain = ' AND tech_type not in %s' % (str(exclude_tech_type).replace('[', '(').replace(']', ')'))
        query = f"""
        SELECT count(*) * 8
        FROM en_technical_model
    ...
    def convert_daterange_to_count(self, employee, start_date, end_date, exclude_tech=None, exclude_tech_type=['off', 'holiday', 'not_work', 'layoff']):
        if start_date > end_date:
            return 0
        exclude_tech_domain = ''
        if exclude_tech:
    ...
    def auto_create_technical_compute(self, employees=None, start_date=None, end_date=None):
        if not employees:
        ...
        for employee in employees:
            i += 1
            for d in date_utils.date_range(start_date, end_date, relativedelta(days=1)):
                tech = self.search_count([('employee_id', '=', employee.id), ('date', '=', d.date())])
                if not tech:
                    tech = self.create({'employee_id': employee.id, 'date': d.date()})
            log.info(f'done auto_create_technical_compute {employee} {i}/{len(employees)}')
            self.env.cr.commit()

    def manual_create_technical_compute(self, employees=None, start_date=None, end_date=None):
        if not employees:
            employees = self.env['hr.employee'].with_context(active_test=False).search([])
        if not start_date:
            start_date = datetime(2020, 1, 1, 0, 0, 0)
        ...
        for employee in employees:
            ...
            for d in date_utils.date_range(start_date, end_date, relativedelta(days=1)):
                ...
                if not tech:
                    tech = self.create({'employee_id': employee.id, 'date': d.date()})
            log.info(f'done manual_create_technical_compute {employee} {i}/{len(employees)}')
    ...
    def count_net_working_days_by_months(self, date_start, date_end):
        ...
        query = """
                WITH date_range AS (SELECT generate_series(%s::date, %s::date, interval '1 day') ::date \
                    AS day
                    ) \
                   , holidays AS (
                ...
                """
        self.env.cr.execute(query, [str(first_day_of_month), str(last_day_of_month), str(first_day_of_month), str(last_day_of_month),])
...
class HrEmployeeLayoff(models.TransientModel):
    ...
    state = fields.Selection(string='Trạng thái', selection=[('inactive', 'Nghỉ việc'), ('semi-inactive', 'Nghỉ không lương'), ('maternity-leave', 'Nghỉ thai sản')], default='inactive', required=True)
    ...
    def onchange_departure_date(self):
        if self.state == 'inactive' and self.employee_id:
            if (not self.employee_id.en_date_start and self.date) or (self.employee_id.en_date_start and self.date and self.employee_id.en_date_start > self.date):
                return {'warning': {
                    'title': 'Lỗi xác nhận',
    ...
    def onchange_day_layoff_from(self):
        if self.state in ['semi-inactive', 'maternity-leave'] and self.employee_id:
            ...
            if self.state == 'maternity-leave':
                text = 'Nghỉ thai sản'
            if (not self.employee_id.en_date_start and self.day_layoff_from) or (self.employee_id.en_date_start and self.day_layoff_from and self.employee_id.en_date_start > self.day_layoff_from):
                return {'warning': {
                    'title': 'Lỗi xác nhận',
    ...
    def do(self):
        ...
        if self.state == 'inactive':
            ...
            self.env['resource.calendar.leaves'].sudo().search([('name', '=', _("%s: %s", self.employee_id.name, text))]).unlink()
            vals_list = [{
                'name': _("%s: %s" % (self.employee_id.name, text)),
                'date_from': timezone(self.env.user.tz or 'UTC').localize(datetime.combine(date_start, time.min)).astimezone(UTC).replace(tzinfo=None),
                'date_to': timezone(self.env.user.tz or 'UTC').localize(datetime.combine(date_end, time.max)).astimezone(UTC).replace(tzinfo=None),
...
class HrEmployeeBase(models.AbstractModel):
    ...
    lock_create_timesheet = fields.Datetime(string='Ngày khóa sổ khai Timesheet', readonly=True, copy=False)
    lock_approve_timesheet = fields.Datetime(string='Ngày khóa sổ duyệt Timesheet', readonly=True, copy=False)
    lock_create_timesheet_exp = fields.Datetime(string='Hạn Ngày khóa sổ khai Timesheet', readonly=True, copy=False)
    lock_approve_timesheet_exp = fields.Datetime(string='Hạn Ngày khóa sổ duyệt Timesheet', readonly=True, copy=False)


class HrEmployee(models.Model):
    _inherit = 'hr.employee'

    technical_field_27774 = fields.One2many(string='🪙', comodel_name='en.resource.detail', inverse_name='employee_id')
    train_ctv = fields.Char(string='PC Đào tạo/CTV', groups='hr.group_hr_user')
    job_code_id = fields.Many2one(comodel_name='job.code', string='Job code', groups='hr.group_hr_user', required=True, related='job_id.job_code_id')
    level_id = fields.Many2one(comodel_name='hr.level', string='Level', domain="[('parent_id','=', False)]", groups='hr.group_hr_user', required=True, related='job_id.level_id')
    sub_level_id = fields.Many2one(comodel_name='hr.level', string='Sub-Level', domain="[('parent_id','!=', False), ('parent_id', '=?', level_id)]", groups='hr.group_hr_user', required=True, related='job_id.sub_level_id')
    pay_grade_id = fields.Many2one(comodel_name='pay.grade', string='Pay grade', groups='hr.group_hr_manager')
    salary_grade = fields.Many2one(comodel_name='salary.grade', string='Bậc lương', groups='hr.group_hr_manager')
    official_working_day = fields.Date(string='Ngày làm việc chính thức', groups='hr.group_hr_user')
    seniority_date = fields.Char(string='Thâm niên', compute='compute_seniority_date', groups='hr.group_hr_user')
    rest_state = fields.Selection([('inactive', 'Nghỉ việc'), ('semi-inactive', 'Nghỉ không lương'), ('maternity-leave', 'Nghỉ thai sản')], string='Trạng thái nghỉ', groups='hr.group_hr_user', compute='compute_rest_state')
    date_start_training = fields.Date(string='Ngày bắt đầu đào tạo/thực tập/CTV', groups='hr.group_hr_user')
    date_end_training = fields.Date(string='Ngày kết thúc đào tạo/thực tập/CTV', groups='hr.group_hr_user')
    date_start_probation = fields.Date(string='Ngày bắt đầu thử việc', groups='hr.group_hr_user')
    date_end_probation = fields.Date(string='Ngày kết thúc thử việc', groups='hr.group_hr_user')
    salary_gross_offer = fields.Integer(string='Lương Gross Offer', groups='hr.group_hr_user')
    salary_probation = fields.Integer(string='Lương Thử việc', groups='hr.group_hr_user')
    salary_basic = fields.Integer(string='Lương cơ bản', groups='hr.group_hr_user')
    lunch_allowance = fields.Integer(string='Trợ cấp ăn trưa', groups='hr.group_hr_user')
    travel_allowance = fields.Integer(string='Trợ cấp đi lại', groups='hr.group_hr_user')
    salary_temporary_effective = fields.Integer(string='Lương hiệu quả tạm tính', groups='hr.group_hr_user')
    total_new_income = fields.Integer(string='Tổng thu nhập mới', groups='hr.group_hr_user')
    total_old_income = fields.Integer(string='Tổng thu nhập cũ', groups='hr.group_hr_user')
    pti_insurance_level = fields.Integer(string='Mức bảo hiểm PTI', groups='hr.group_hr_user')
    ...
    depend_persion_id = fields.One2many(comodel_name='depend.persion', string='Người phụ thuộc/Quan hệ nhân thân', inverse_name='hr_employee_id', groups='hr.group_hr_user')
    regular_address = fields.Char(string='Địa chỉ thường trú', groups='hr.group_hr_user', required=False)
    cccd_date = fields.Date(string='Ngày cấp CCCD', groups='hr.group_hr_user', required=False)
    cccd_place = fields.Char(string='Nơi cấp CCCD', groups='hr.group_hr_user', required=False)
    ...
    indirect_manager = fields.Many2one('hr.employee', string='Quản lý gián tiếp', groups='hr.group_hr_user')
    check_representative = fields.Boolean(string='Người đại diện', groups='hr.group_hr_user')
    address_current = fields.Char(string='Địa chỉ hiện tại', required=False, groups='hr.group_hr_user')
    res_partner_bank_ids = fields.One2many(string='Ngân hàng', comodel_name='res.partner.bank', inverse_name='hr_employee_id', groups='hr.group_hr_user')
    private_email = fields.Char(readonly=False)
    state_hr_employee = fields.Selection([('permanent', 'Chính thức'), ('probation', 'Thử việc'), ('training', 'Đào tạo'), ('inter', 'Thực tập'), ('maternity','Thai sản'), ('semi-inactive', 'Nghỉ không lương'), ('contract_lease', 'Thuê khoán')], string='Tình trạng', groups='hr.group_hr_user')
    old_state_hr_employee = fields.Char(string='Tình trạng cũ', groups='hr.group_hr_user', readonly=1, copy=False)
    shift = fields.Selection([('ca_vip', 'Ca VIP'), ('ca_thuong', 'Ca Thường'), ('ca_linh_hoat', 'Ca linh hoạt')], default='ca_thuong', string='Ca làm việc', groups='hr.group_hr_user')
    resource_project_ids = fields.One2many(string='Danh sách nhân sự', comodel_name='resource.project', inverse_name='employee_id', groups='hr.group_hr_user')
    ...
    @api.depends('en_level_id', 'en_level_id.sequence', 'parent_id', 'parent_id.en_level_id', 'parent_id.en_level_id.sequence', 'en_department_id', 'en_department_id.manager_id')
    def _compute_parent_path_chart(self):
        for employee in self:
            ...
            if min_sequence < employee.en_level_id.sequence:
                parent_path_chart += '/'.join([''] + ['False LV' + str(i) for i in range(min_sequence, employee.en_level_id.sequence)])
    ...
    def check_constrains_representative(self):
        for rec in self:
            if rec.check_representative:
                exit_representative = self.search([('check_representative', '=', True), ('id', '!=', rec.id)])
    ...
    def compute_seniority_date(self):
        for rec in self:
            if not rec.official_working_day or not isinstance(rec.official_working_day, date):
                rec.seniority_date = f'0 Ngày'
                continue
            else:
                if rec.departure_date:
                    seniority_date = relativedelta(rec.departure_date, rec.official_working_day)
                else:
    ...
    def check_date_probation(self):
        if self.date_end_probation and self.date_start_probation and self.date_start_probation > self.date_end_probation:
            raise UserError('Ngày bắt đầu thử việc phải nhỏ hơn ngày kết thúc thử việc')
    ...
    def action_departure_pause_employee(self):
        ...
        local_time = utc_time.replace(tzinfo=dt.timezone.utc).astimezone(to_zone).date()
    ...
    def action_departure_employee(self, local_time):
        employee_departure = self.search([('departure_date', '=', local_time), ('en_status', 'not in', ['inactive'])])
    ...
    def action_departure(self):
        for employee in self:
            resource_project = self.env['resource.project'].search([
                ('employee_id', '=', employee.id), ('date_start', '<=', employee.departure_date),
    ...
    def action_pause_employee(self, local_time):
        employee_pause = self.search([('en_day_layoff_from', '=', local_time), ('en_status', '=', 'active')])
        for employee in employee_pause:
            resource_project = self.env['resource.project'].search([
                ('employee_id', '=', employee.id), ('date_start', '<=', employee.en_day_layoff_from),
    ...
    def cron_create_birthday_allocation(self):
        ...
        leave_type = self.env['hr.leave.type'].search([('code', '=', 'SN')], limit=1)
        if not leave_type:
            return
        for employee in self.search([('state_hr_employee', '=', 'permanent')]):
            ...
            if employee.birthday.month != today.month:
                continue
            if not self.env['hr.leave.allocation'].search([('employee_id', '=', employee.id), ('holiday_status_id', '=', leave_type.id), ('date_from', '>=', today + relativedelta(day=1, month=1)), ('date_from', '<=', today + relativedelta(day=31, month=12))]):
    ...
    def action_change_date_end(self, local_time):
        resource_details = self.env['en.resource.detail'].search([
            ('employee_id', '=', self.id), ('date_start', '<', local_time), ('date_end', '>=', local_time)
        ])
        for resource in resource_details:
            message = f'''
    ...
    def _constrains_departure_date_en_date_start(self):
        if any((not rec.en_date_start and rec.departure_date) or (rec.en_date_start and rec.departure_date and rec.en_date_start > rec.departure_date) for rec in self):
            raise exceptions.ValidationError('Ngày dừng hoạt động nhân viên không thể nhỏ hơn ngày bắt đầu làm việc của nhân viên')

    @api.constrains('en_date_start', 'en_day_layoff_from')
    def _constrains_en_day_layoff_from_en_date_start(self):
        if any((not rec.en_date_start and rec.en_day_layoff_from) or (rec.en_date_start and rec.en_day_layoff_from and rec.en_date_start > rec.en_day_layoff_from) for rec in self):
            raise exceptions.ValidationError('Ngày tạm dừng hoạt động của nhân viên không thể nhỏ hơn ngày bắt đầu làm việc của nhân viên.')
    ...
    def default_get(self, fields_list):
        ...
        if 'role_ids' in fields_list:
            res['role_ids'] = self.env['entrust.role'].search([('name', '=', 'Thành viên dự án')]).ids
        if self.env.context.get('hidden'):
            res['en_type_id'] = self.env['en.type'].search([('is_hidden', '=', True)], limit=1).id
        return res

    def button_resource_account_report_wizard_act(self):
        return self.open_form_or_tree_view('account_reports.resource_account_report_wizard_act', False, False, {'default_employee_id': self.id}, 'Thông tin nguồn lực', 'new')

    def unlink(self):
        expt = []
        for rec in self:
            resources = self.env['en.resource.detail'].search([('employee_id', '=', rec.id)])
            if not resources: continue
            expt += [f'Nhân viên {rec.display_name} đang ở trong kế hoạch nguồn lực của dự án {", ".join(list(set(resources.mapped("order_id.project_id.display_name"))))}']
    ...
    def recompute_en_status(self):
        ...
        for rec in self:
            ...
            if rec.en_day_layoff_to and rec.en_day_layoff_to < fields.Date.today() and rec.en_status != 'active':
                rec.with_context(recompute_en_status=True).write({'en_status': 'active'})
    ...
    def en_button_layoff(self):
        record = self.env['hr.employee.layoff.wizard'].create({'employee_id': self.id})
        return self.open_form_or_tree_view('ngsd_base.hr_employee_layoff_wizard_act', False, record, {}, 'Nghỉ việc', 'new')

    en_text_off = fields.Text(string='Lý do nghỉ việc', readonly=True, copy=False, groups="hr.group_hr_user")
    en_day_layoff_from = fields.Date(string='Ngày bắt đầu tạm dừng làm việc', readonly=False, copy=False, groups="hr.group_hr_user")
    en_day_layoff_to = fields.Date(string='Ngày kết thúc tạm dừng làm việc', readonly=False, copy=False, groups="hr.group_hr_user")
    en_text_layoff = fields.Text(string='Lý do nghỉ dài hạn', readonly=False, copy=False, groups="hr.group_hr_user")

    _sql_constraints = [
        ('barcode_uniq', 'unique (id)', "The Badge ID must be unique, this one is already assigned to another employee."),
    ]

    @api.constrains('barcode', 'work_email', 'en_type_id')
    def _ngsd_constrains_duplicate(self):
        for rec in self:
            if rec.en_internal_ok and self.search_count([('barcode', '=', rec.barcode)]) > 1:
                raise exceptions.ValidationError('Mã nhân sự/ Email bị trùng, vui lòng kiểm tra lại!')
            if rec.en_internal_ok and self.search_count([('work_email', '=', rec.work_email)]) > 1:
                raise exceptions.ValidationError('Mã nhân sự/ Email bị trùng, vui lòng kiểm tra lại!')
    ...
    en_status = fields.Selection(string='Trạng thái', groups="hr.group_hr_user", selection=[('active', 'Hoạt động'), ('inactive', 'Nghỉ việc'), ('semi-inactive', 'Nghỉ dài hạn')], default='active', copy=False, required=True)
    en_status_hr = fields.Selection(string='Trạng thái', groups="hr.group_hr_user", selection=[('active', 'Hoạt động'), ('inactive', 'Nghỉ việc'), ('semi-inactive', 'Nghỉ không lương'), ('maternity-leave', 'Nghỉ thai sản')], default='active', store=True, copy=False, compute='_get_en_status_hr', inverse='_inverse_en_status_hr')
    ...
    en_type_id = fields.Many2one(required=False, string='Loại', comodel_name='en.type', groups="hr.group_hr_user")
    # Lấy giá trị is_intern từ en_type_id
    is_intern = fields.Boolean(string='Thực tập sinh', related='en_type_id.is_intern', readonly=True, store=True)
    en_internal_ok = fields.Boolean(related='en_type_id.internal_ok', groups="hr.group_hr_user")
    is_hidden = fields.Boolean(related='en_type_id.is_hidden', groups="hr.group_hr_user")
    en_area_id = fields.Many2one(required=False, string='Khu vực', comodel_name='en.name.area', groups="hr.group_hr_user")
    en_date_start = fields.Date(required=False, string='Ngày bắt đầu', default=lambda self: fields.Date.today(), groups="hr.group_hr_user")
    en_block_id = fields.Many2one(required=False, string='Khối', comodel_name='en.name.block', groups="hr.group_hr_user", domain="['|', ('area_id', '=?', en_area_id),('en_area_ids', '=', en_area_id)]")
    job_title = fields.Char(readonly=True)
    en_department_id = fields.Many2one(string='Phòng', comodel_name='en.department', groups="hr.group_hr_user", domain="[('department_id', '=', department_id)]")
    en_level_id = fields.Many2one(required=False, string='Cấp bậc', comodel_name='en.name.level', groups="hr.group_hr_user")
    en_technique = fields.Char(required=False, string='Kỹ năng', groups="hr.group_hr_user")
    en_date_from = fields.Date(readonly=True, copy=False, string='Ngày bắt đầu', groups="hr.group_hr_user")
    en_date_to = fields.Date(readonly=True, copy=False, string='Ngày kết thúc', groups="hr.group_hr_user")

    target_quarter = fields.Float(string="Chỉ tiêu quý", default=0, groups="hr.group_hr_user")
    target_year = fields.Float(string="Chỉ tiêu năm", default=0, groups="hr.group_hr_user")

    start_date_quarter = fields.Date(string="Áp dụng từ ngày", groups="hr.group_hr_user")
    end_date_quarter = fields.Date(string="Áp dụng đến ngày", groups="hr.group_hr_user")

    start_date_year = fields.Date(string="Áp dụng từ ngày", groups="hr.group_hr_user")
    end_date_year = fields.Date(string="Áp dụng đến ngày", groups="hr.group_hr_user")
    ...
    def get_revenue_quarter(self):
        ...
        domain = [('user_id', 'in', user_ids.ids), ('date_deadline', '>=', self.start_date_quarter), ('date_deadline', '<=', self.end_date_quarter)]
        total_revenue = sum(self.env['crm.lead'].search(domain).mapped('total_revenue'))
        return total_revenue

    def get_revenue_year(self):
        user_ids = self.env.user | self.subordinate_ids.user_id
        domain = [('user_id', 'in', user_ids.ids), ('date_deadline', '>=', self.start_date_year), ('date_deadline', '<=', self.end_date_year)]
        total_revenue = sum(self.env['crm.lead'].search(domain).mapped('total_revenue'))
        return total_revenue
    ...
    def noti_not_target_quarter(self):
        ...
        for emp in emps:
            ...
            if total_revenue < emp.target_quarter:
                ...
                emp.send_notify(mes, emp.user_id, link=link, model_description="Dashboard")
        return {
    ...
    def create(self, vals):
        lock_create_timesheet = self.env['lock.license'].get_lasted_date_lock('create')
        if lock_create_timesheet:
            vals['lock_create_timesheet'] = lock_create_timesheet
        lock_approve_timesheet = self.env['lock.license'].get_lasted_date_lock('approve')
        if lock_approve_timesheet:
            vals['lock_approve_timesheet'] = lock_approve_timesheet
        fields = ['target_quarter', 'target_year', 'start_date_quarter', 'end_date_quarter', 'start_date_year', 'end_date_year']
        match_fields = list(set(fields) & set(vals.keys()))
        res = super().create(vals)
        if not res.user_id and res.work_email:
            res.user_id = self.env['res.users'].sudo().create({
                'name': res.name,
                'email': res.work_email,
                'login': res.work_email,
                'image_1920': res.image_1920,
                'role_ids': [(6, 0, res.role_ids.ids)],
                'tz': res.tz,
            })
        if match_fields and res.parent_id:
            res.parent_id.update_total_target_quarter(res.start_date_quarter, res.end_date_quarter)
            res.parent_id.update_total_target_year(res.start_date_year, res.end_date_year)
        self.env['en.technical.model'].manual_create_technical_compute(res, datetime.now() + relativedelta(years=-1, month=1, day=1, hour=0, minute=0, second=0), datetime.now() + relativedelta(years=2, month=12, day=31, hour=23, minute=59, second=59))
        return res
    ...
    def write(self, vals):
        ...
        if 'state_hr_employee' in vals:
            for rec in self:
                if rec.en_status == 'semi-inactive' and rec.state_hr_employee in ['maternity', 'semi-inactive']:
                    raise UserError("Bạn không thể đổi Tình trạng khi đang trong thời gian Nghỉ không lương/Nghỉ thai sản")
        if len(self) == 1:
        ...
        if 'departure_date' in vals:
            for rec in self:
                if rec.departure_date and rec.departure_date <= fields.Date.Date.context_today(self):
                    rec.action_departure()

        if self._context.get('no_compute_parent'):
            return res
        lst_fields = ['target_quarter', 'target_year', 'start_date_quarter', 'end_date_quarter', 'start_date_year', 'end_date_year']
        match_fields = list(set(lst_fields) & set(vals.keys()))
        for rec in self:
            if match_fields and rec.parent_id:
                rec.parent_id.update_total_target_quarter(rec.start_date_quarter, rec.end_date_quarter)
                rec.parent_id.update_total_target_year(rec.start_date_year, rec.end_date_year)
        if 'resource_calendar_id' in vals:
            for rec in self:
                miss_match = self.env['resource.calendar.leaves'].sudo().search([('resource_id.user_id', '=', rec.user_id.id), ('calendar_id', '!=', rec.resource_calendar_id.id)])
                if miss_match:
    ...
    def _create_future_public_holidays_timesheets(self, employees):
        self = self.with_context(no_constrains=True)
        return super(HrEmployee, self)._create_future_public_holidays_timesheets(employees)

    @api.constrains('en_type_id', 'barcode', 'work_email', 'en_area_id', 'en_block_id', 'department_id', 'job_id', 'en_level_id', 'en_date_start')
    def check_required_field(self):
        fields_check = ['work_email', 'department_id', 'job_id', 'en_level_id', 'en_date_start']
        ...
        for rec in self:
            ...
            if not rec.en_type_id.is_os and any(not rec[f] for f in fields_check_not_os):
                raise UserError('Thiếu thông tin bắt buộc')

    check_timesheet_before_checkout = fields.Boolean('Bắt buộc khai timesheet', groups='hr.group_hr_user', default=True)

    def get_hour_working_by_day(self, day):
        # thời gian không làm việc
        tech_data = self.env['en.technical.model'].convert_daterange_to_data(self, day, day)
        if tech_data[day].get('tech_type') in ['off', 'not_work', 'layoff', 'holiday']:
            return -1
        calendar = self.resource_calendar_id or self.env.company.resource_calendar_id
        # # thời gian nghỉ phép
        # leave_hour = 8 - tech_data[day].get('number')
        # Khai ts
        ts_hour = sum(self.env['account.analytic.line'].search([('employee_id', '=', self.id), ('date', '=', day), ('en_state', 'in', ['approved'])]).mapped('unit_amount'))
        # Khai nghỉ phép
        hr_leave = self.env['hr.leave'].search([('employee_id', '=', self.id), ('request_date_from', '<=', day), ('request_date_to', '>=', day), ('state', 'in', ['validate']), ('holiday_status_id.code', '!=', 'W')])
        leave_hour = sum([min(max(l.number_of_hours_display, l.number_of_days * calendar.hours_per_day), 8) for l in hr_leave])
        return ts_hour + leave_hour

    role_ids = fields.Many2many('entrust.role', string='Vai trò phân quyền', required=0)

    @api.model
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        args = args or []
        employee_ids = []
        if self._context.get('view_all_hr_employee') and self.env.user.has_group('ngsd_base.group_hcns'):
            self = self.sudo()
        ...
        if not employee_ids:
            employee_ids = self._search(expression.AND([[('name', operator, name)], args]), limit=limit, access_rights_uid=name_get_uid)
        return employee_ids

    timesheet_general_ids = fields.One2many('timesheet.general', 'employee_id', 'Bảng tổng hợp chấm công')
...
class DependPerson(models.Model):
    ...
    relationship = fields.Selection([('parent', 'Cha Mẹ'), ('child', 'Con'), ('couple', 'Vợ/Chồng'), ('other', 'Khác')], string='Quan hệ', required=True)
    ...
    hr_employee_id = fields.Many2one(comodel_name='hr.employee', string='Nhân viên', required=True, ondelete='cascade')
...
class HrResumeLineInheit(models.Model):
    ...
    hr_resume_type = fields.Selection(related='line_type_id.hr_resume_type', string='Loại sơ yếu lý lịch')
...
class HrResumeLineType(models.Model):
    ...
    hr_resume_type = fields.Selection([('exp', 'Experience'), ('edu', 'Education')], required=True, string='Loại sơ yếu lý lịch')
...
class ResPartnerBank(models.Model):
    ...
    hr_employee_id = fields.Many2one(comodel_name='hr.employee', string='Nhân viên')
...
class TimesheetGeneral(models.Model):
    ...
    def init_data(self, employee):
        ...
        values = []
        ts_new = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('holiday_id', '=', False), ('en_state', '=', 'new'), '|', ('project_id.active', '=', True), ('en_nonproject_task_id', '!=', False), ], ['unit_amount'], ['date:day'])
        for ts_n in ts_new:
            text_context_new = """"""
            ts_new_p = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('en_state', '=', 'new'), ('project_id.active', '=', True), ('date', '=', ts_n['__range']['date']['from'])], ['unit_amount'], ['project_id'])
            ts_new_np = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('en_state', '=', 'new'), ('en_nonproject_task_id', '!=', False), ('date', '=', ts_n['__range']['date']['from'])], ['unit_amount'], ['en_nonproject_task_id'])
            for ts_new in ts_new_p:
                if ts_new['unit_amount'] > 0:
                    text_context_new += f"""  {self.env['project.project'].browse(ts_new['project_id'][0]).en_code} : {round(ts_new['unit_amount'], 2)} giờ \n""" if ts_new['project_id'] else f"""  : {round(ts_new['unit_amount'], 2)} giờ \n"""
            for ts_new_n in ts_new_np:
                if ts_new_n['unit_amount'] > 0:
                    text_context_new += f"""  {self.env['en.nonproject.task'].browse(ts_new_n['en_nonproject_task_id'][0]).en_department_id.name} : {round(ts_new_n['unit_amount'], 2)} giờ \n""" if ts_new_n['en_nonproject_task_id'] else f"""  : {round(ts_new_n['unit_amount'], 2)} giờ \n"""
            values.append({
        ...
        ts_pending = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('en_state', '=', 'sent'), '|', ('project_id.active', '=', True), ('en_nonproject_task_id', '!=', False), ], ['unit_amount'], ['date:day'])
        for ts_p in ts_pending:
            text_context_pending = """"""
            ts_pending_p = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('en_state', '=', 'sent'), ('project_id.active', '=', True), ('date', '=', ts_p['__range']['date']['from'])], ['unit_amount'], ['project_id'])
            ts_pending_np = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('en_state', '=', 'sent'), ('en_nonproject_task_id', '!=', False), ('date', '=', ts_p['__range']['date']['from'])], ['unit_amount'], ['en_nonproject_task_id'])
            for ts_pending in ts_pending_p:
                if ts_pending['unit_amount'] > 0:
                    text_context_pending += f"""  {self.env['project.project'].browse(ts_pending['project_id'][0]).en_code} : {round(ts_pending['unit_amount'], 2)} giờ \n""" if ts_pending['project_id'] else f"""  : {round(ts_pending['unit_amount'], 2)} giờ \n"""
            for ts_pending_n in ts_pending_np:
                if ts_pending_n['unit_amount'] > 0:
                    text_context_pending += f"""  {self.env['en.nonproject.task'].browse(ts_pending_n['en_nonproject_task_id'][0]).en_department_id.name} : {round(ts_pending_n['unit_amount'], 2)} giờ \n""" if ts_pending_n['en_nonproject_task_id'] else f"""  : {round(ts_pending_n['unit_amount'], 2)} giờ \n"""
            values.append({
        ...
        ts_approved = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('en_state', '=', 'approved'), '|', ('project_id.active', '=', True), ('en_nonproject_task_id', '!=', False), ], ['unit_amount', 'date:max'], ['date:day'])
        for ts_a in ts_approved:
            text_context_approved = """"""
            ts_approved_p = self.env['account.analytic.line'].read_group(
                [('employee_id', '=', employee), ('en_state', '=', 'approved'), ('project_id.active', '=', True),
                 ('date', '=', ts_a['__range']['date']['from'])], ['unit_amount'], ['project_id'])
            ts_approved_np = self.env['account.analytic.line'].read_group(
                [('employee_id', '=', employee), ('en_state', '=', 'approved'), ('en_nonproject_task_id', '!=', False),
                 ('date', '=', ts_a['__range']['date']['from'])], ['unit_amount'], ['en_nonproject_task_id'])
            for ts_a_p in ts_approved_p:
                if ts_a_p['unit_amount'] > 0:
                    ...
                    ts_a_p['project_id'] else f"""  : {round(ts_a_p['unit_amount'], 2)} giờ \n"""
            for ts_a_n in ts_approved_np:
                if ts_a_n['unit_amount'] > 0:
                    ...
                    ts_a_n['en_nonproject_task_id'] else f"""  : {round(ts_a_n['unit_amount'], 2)} giờ \n"""
            values.append({
        ...
        ot_pending = self.env['account.analytic.line'].read_group(
            [('employee_id', '=', employee), ('ot_state', '=', 'requested'), '|', ('project_id.active', '=', True),
             ('en_nonproject_task_id', '!=', False), ], ['ot_time'], ['date:day'])
        for ot_p in ot_pending:
            text_context_pending_ot = """"""
            ot_pending_p = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('ot_state', '=', 'requested'), ('project_id.active', '=', True), ('date', '=', ot_p['__range']['date']['from'])], ['ot_time'], ['project_id'])
            ot_pending_np = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('ot_state', '=', 'requested'), ('en_nonproject_task_id', '!=', False), ('date', '=', ot_p['__range']['date']['from'])], ['ot_time'], ['en_nonproject_task_id'])
            for ot_pending in ot_pending_p:
                if ot_pending['ot_time'] > 0:
                    text_context_pending_ot += f"""  {self.env['project.project'].browse(ot_pending['project_id'][0]).en_code} : {round(ot_pending['ot_time'], 2)} giờ \n""" if ot_pending['project_id'] else f"""  : {round(ot_pending['ot_time'], 2)} giờ \n"""
            for ot_pending_n in ot_pending_np:
                if ot_pending_n['ot_time'] > 0:
                    text_context_pending_ot += f"""  {self.env['en.nonproject.task'].browse(ot_pending_n['en_nonproject_task_id'][0]).en_department_id.name} : {round(ot_pending_n['ot_time'], 2)} giờ \n""" if ot_pending_n['en_nonproject_task_id'] else f"""  : {round(ot_pending_n['ot_time'], 2)} giờ \n"""
            values.append({
        ...
        ot_approved = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('ot_state', '=', 'approved'), '|', ('project_id.active', '=', True), ('en_nonproject_task_id', '!=', False), ], ['ot_time'], ['date:day'])
        for ot in ot_approved:
            text_context_ot_approved = """"""
            ot_approved_p = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('ot_state', '=', 'approved'), ('project_id.active', '=', True), ('date', '=', ot['__range']['date']['from'])], ['ot_time'], ['project_id'])
            ot_approved_np = self.env['account.analytic.line'].read_group([('employee_id', '=', employee), ('ot_state', '=', 'approved'), ('en_nonproject_task_id', '!=', False), ('date', '=', ot['__range']['date']['from'])], ['ot_time'], ['en_nonproject_task_id'])
            for ot_a_p in ot_approved_p:
                if ot_a_p['ot_time'] > 0:
                    text_context_ot_approved += f"""  {self.env['project.project'].browse(ot_a_p['project_id'][0]).en_code} : {round(ot_a_p['ot_time'], 2)} giờ \n""" if ot_a_p['project_id'] else f"""  : {round(ot_a_p['ot_time'], 2)} giờ \n"""
            for ot_a_n in ot_approved_np:
                if ot_a_n['ot_time'] > 0:
                    text_context_ot_approved += f"""  {self.env['en.nonproject.task'].browse(ot_a_n['en_nonproject_task_id'][0]).en_department_id.name} : {round(ot_a_n['ot_time'], 2)} giờ \n""" if ot_a_n['en_nonproject_task_id'] else f"""  : {round(ot_a_n['ot_time'], 2)} giờ \n"""
            values.append({
        ...
        plan_ot_pending = self.env['en.overtime.plan'].read_group([('create_uid', '=', user_id), ('state', '=', 'to_approve')], ['en_hours'], ['en_date:day'])
        for plan_ot in plan_ot_pending:
            text_context_plan = """"""
            plan_ot_pending_p = self.env['en.overtime.plan'].read_group([('create_uid', '=', user_id), ('state', '=', 'to_approve'), ('en_work_inproject', '=', True), ('en_date', '=', plan_ot['__range']['en_date']['from'])], ['en_hours'], ['en_project_id'])
            plan_ot_pending_np = self.env['en.overtime.plan'].read_group([('create_uid', '=', user_id), ('state', '=', 'to_approve'), ('en_work_inproject', '=', False), ('en_date', '=', plan_ot['__range']['en_date']['from'])], ['en_hours'], ['en_work_nonproject_id'])
            for plan_ot_p in plan_ot_pending_p:
                if plan_ot_p['en_hours'] > 0:
                    text_context_plan += f"""  {self.env['project.project'].browse(plan_ot_p['en_project_id'][0]).en_code} : {round(plan_ot_p['en_hours'], 2)} giờ \n""" if plan_ot_p['en_project_id'] else f"""  : {round(plan_ot_p['en_hours'], 2)} giờ \n"""
            for plan_ot_np in plan_ot_pending_np:
                if plan_ot_np['en_hours'] > 0:
                    text_context_plan += f"""  {self.env['en.nonproject.task'].browse(plan_ot_np['en_work_nonproject_id'][0]).en_department_id.name} : {round(plan_ot_np['en_hours'], 2)} giờ \n""" if plan_ot_np['en_work_nonproject_id'] else f"""  : {round(plan_ot_np['en_hours'], 2)} giờ \n"""
            values.append({
        ...
        leave_approved = self.env['en.technical.model'].read_group([('employee_id', '=', employee), ('tech', '=', 'leave'), ('tech_type', '=', 'leave')], ['number'], ['date:day'])
        ...
        for leave in leaves:
            ...
            for day in date_utils.date_range(leave.date_from, leave.date_to, relativedelta(days=1)):
        ...
        leave_holiday = self.env['resource.calendar.leaves'].search([('is_holiday', '=', True)])
        for holiday in leave_holiday:
            for date_step in date_utils.date_range(holiday.date_from, holiday.date_to, relativedelta(days=1)):
                date_step = (datetime.combine(date_step, time.min) + relativedelta(hours=7)).date()
                values.append({
                    'date': date_step,
                    'employee_id': employee,
                    'total_time': 8,
