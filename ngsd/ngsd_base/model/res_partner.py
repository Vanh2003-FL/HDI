...
class ResPartner(models.Model):
    ...
    bfsi_id = fields.Many2one(string='Phân khúc khách hàng BFSI', comodel_name='en.bfsi')
    technical_field_27310 = fields.Many2many(string='🪙', relation='partner_with_lead_partner_rel', column1='src_id', column2='dest_id', comodel_name='res.partner', readonly=True, copy=False)

    en_internal = fields.Boolean(string='Nội bộ', default=False, readonly=True, copy=False)
    en_from_lead_ok = fields.Boolean(string='Tạo từ cơ hội', default=False, readonly=True, copy=False)
    x_code = fields.Char(string='Mã khách hàng')
    x_short = fields.Char(string='Tên viết tắt')
    x_gender = fields.Selection(string='Giới tính', selection=[('male', 'Nam'), ('female', 'Nữ'), ('other', 'Khác')])
    ...
    activity_user_ids = fields.Many2many('res.users', string="Người phụ trách", relation='activity_user_ids', default=lambda self: [(6, 0, self.env.user.ids)])
    ...
    company_type_res = fields.Char(related='company_id.company_type', string="Loại công ty")
    customer_type = fields.Selection(related='type', string='Address Type', readonly=True, )
    customer_category_id = fields.Many2one('x.customer.category', string='Mảng khách hàng', copy=False)
    customer_code = fields.Char(string='Mã khách hàng', copy=False, readonly=True)
    customer_group_id = fields.Many2one('x.customer.group', string='Nhóm khách hàng', copy=False)
    x_note = fields.Text(string='Mô tả')
    x_dob = fields.Date(string='Ngày sinh')

    is_employee = fields.Boolean('Nhân viên', compute='_compute_is_employee', store=True, compute_sudo=True)

    def _compute_is_employee(self):
        for rec in self:
            if rec.user_ids.employee_id:
    ...
    def check_unique_vat(self):
        for rec in self:
            ...
            domain = [('id', 'not in', child_ids.ids), ('vat', '=', rec.vat), ('company_id', '=', rec.company_id.id)]
            if rec.is_customer:
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        if view_type == 'tree':
            ncs_company_ids = self.env['res.company'].search([('company_type', '=', 'ncs')]).ids
            if ncs_company_ids and len(set(self._context.get('allowed_company_ids', False) + ncs_company_ids)) != len(self._context.get('allowed_company_ids', False) + ncs_company_ids):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('x_code'):
                vals['x_code'] = self.env['ir.sequence'].next_by_code('seq.code.partner.ac')
            if vals.get('is_customer'):
                vals['customer_code'] = self.env['ir.sequence'].next_by_code('ngsd.base.res.partner')
        ...
        for rec in res:
            rec.activity_user_ids = [(6, 0, (rec.activity_user_ids.ids or []) + (rec.parent_id.activity_user_ids.ids or []))]
        return res
    ...
    def _name_search(self, name='', args=None, operator='ilike', limit=100, name_get_uid=None):
        if self._context.get('view_all_res_partner'):
            self = self.sudo()
        return super(ResPartner, self)._name_search(name=name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
