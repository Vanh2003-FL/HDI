...
class ResourcePlanning(models.Model):
  ...
  en_md = fields.Float(string='Tổng nguồn lực (MD)', compute_sudo=True,
                       compute='_compute_en_m_uom', store=True)

  @api.depends('order_line.en_md', 'project_id.en_history_resource_ids',
               'project_id.mm_rate')
  def _compute_en_m_uom(self):
    for rec in self:
      rec.en_md = sum(rec.order_line.mapped('en_md')) + sum(
        rec.project_id.en_history_resource_ids.mapped(
          'plan')) * rec.project_id.mm_rate

  hours_total = fields.Float(string='Tổng nguồn lực (MH)', compute_sudo=True,
                             compute='_compute_hours_total', store=True)
  ...
  @api.depends('project_id', 'project_id.en_resource_ids', 'parent_id',
  ...
  def _compute_version_number(self):
    ...
    for project in self.filtered(
        ...
        "project_id"):
      sequence = 1
      wbs = project.en_resource_ids.filtered(
  ...
  def _compute_parent_id(self):
    for rec in self:
      ...
      if rec.version_type == 'plan':
        parent_id = self.env['en.resource.planning'].search(
  ...
  child_ids = fields.One2many(string='Plan',
                              comodel_name='en.resource.planning',
  ...
  mm_conversion = fields.Float(string='Tổng MM kế hoạch',
                               compute='_get_mm_conversion')
  ...
  def _constrains_workload_gather_0(self):
    for line in self.order_line:
      if not line.workload > 0:
        raise exceptions.ValidationError(
          f'Nhân viên {line.employee_id.display_name} trong khoảng thời gian {line.date_start.strftime("%d/%m/%Y")} → {line.date_end.strftime("%d/%m/%Y")} không thể nhập workload nhỏ hơn hoặc bằng 0')

  @api.model
  def get_project_name(self):
    self = self.env['en.resource.planning'].browse(
  ...
  def _get_employee_data(self, level=0, position=False):
    records = self.order_line.filtered(
  ...
  def _constrains_project_id_state(self):
    if self._context.get('import_file') and any(
        self.env['en.resource.planning'].search_count(
            [('project_id', '=', rec.project_id.id),
             ('state', '=', 'draft')]) > 1 for rec in self):
      raise exceptions.ValidationError(
        f'Bạn không thể tạo hai KHNL ở trạng thái dự kiến ')
  ...
  def _compute_technical_field_27766(self):
    for rec in self:
      ...
      if rec.state not in ['approved', 'refused'] or not rec.project_id:
        ...
        continue
      if self.env['en.resource.planning'].search_count(
          [('project_id', '=', rec.project_id.id),
           ('state', '=', 'draft')]) >= 1:
        rec.technical_field_27766 = technical_field_27766
        continue
      if rec._origin.id == self.env['en.resource.planning'].search(
          [('project_id', '=', rec.project_id.id)], order='id desc',
          limit=1).id or rec._origin.id == self.env[
        'en.resource.planning'].search(
  ...
  def _constrains_overload(self):
    ...
    for rec in self:
      ...
      order_lines = rec.order_line.filtered(
        lambda l: l.employee_id.en_internal_ok)
      if not order_lines:
        continue
      ...
      for detail in resource_details:
        for d in date_utils.date_range(
            datetime.combine(detail.date_start, datetime.min.time()),
            datetime.combine(detail.date_end, datetime.min.time()),
      ...
      expt_txt = [f"{msg} {' và '.join(format_ranges(dates))}" for msg, dates in
                  groupby_overwork.items() if dates]
      if expt_txt:
        raise exceptions.ValidationError('\n'.join(expt_txt))
  ...
  order_line = fields.One2many(string='Chi tiết nguồn lực',
                               states=EDIT_DRAFT_STATES,
                               comodel_name='en.resource.detail',
                               inverse_name='order_id', copy=False)

  resource_total = fields.Float(string='Tổng nguồn lực (MM)', compute_sudo=True,
                                compute='_compute_resource_total')
  resource_total_rate = fields.Float(default=1)
  increase_resource_total = fields.Boolean('Tăng nguồn lực với phiên bản cũ',
                                           compute='_compute_increase_resource_total',
                                           store=True)
  ...
  def _compute_increase_resource_total(self):
    for rec in self:
      ...
      if (rec.project_id.en_resource_id and round(
          rec.project_id.en_resource_id.en_md, 2) < round(rec.en_md,
                                                          2)) or not rec.project_id.en_resource_id:
        rec.increase_resource_total = True

  @api.depends('order_line.mm', 'order_line.workload', 'order_line')
  def _compute_resource_total(self):
    for rec in self:
      rec.resource_total = sum(line.mm * line.workload for line in
                               rec.order_line) * rec.resource_total_rate

  budget_over = fields.Float(string='% vượt mức budget', compute_sudo=True,
                             compute='_compute_budget_over')
  ...
  def _compute_budget_over(self):
    for rec in self:
      ...
      if rec.project_id.en_bmm:
        budget_over = (
                            rec.mm_conversion - rec.project_id.en_bmm) / rec.project_id.en_bmm
      rec.budget_over = max([budget_over, 0])

  baseline_over = fields.Float(string='% vượt mức baseline', compute_sudo=True,
                               compute='_compute_baseline_over')
  ...
  def _compute_baseline_over(self):
    for rec in self:
      ...
      if rec.parent_id.mm_conversion:
        baseline_over = (
                              rec.mm_conversion - rec.parent_id.mm_conversion) / rec.parent_id.mm_conversion
      rec.baseline_over = max([baseline_over, 0])

  confirm_user_approved = fields.Selection(string="Xác định người duyệt",
                                           selection=[
                                             ('v1', 'KHNL max > KHNL new'),
                                             ('v2',
                                              'BMM > KHNL new > KHNL max'),
                                             ('v3',
                                              'KHNL new > BMM > KHNL max'),
                                             ('v4', 'KHNL đầu tiên'),
                                           ], required=False,
                                           compute='_compute_confirm_user_approved',
                                           store=True, compute_sudo=True)

  @api.depends('mm_conversion', 'project_id.en_resource_ids.state',
               'project_id.en_resource_ids')
  def _compute_confirm_user_approved(self):
    for rec in self:
      if not any(resource.state in ['approved', 'expire'] for resource in
                 rec.project_id.en_resource_ids):
        rec.confirm_user_approved = 'v4'
      else:
        if any(
            round(resource.mm_conversion, 2) >= round(rec.mm_conversion, 2) for
            resource in rec.project_id.en_resource_ids.filtered(
                lambda x: x.state in ['approved', 'expire'])):
  ...
  count_approver = fields.Integer(string='Số cấp duyệt',
                                  compute='_count_approver')
...
class ResourceDetail(models.Model):
  ...
  en_used_hour = fields.Float(string='Số giờ đã sử dụng',
                              compute='_compute_en_used_hour')
  old_line_id = fields.Many2one('en.resource.detail', string='Line cũ',
                                readonly=True, copy=False)
  ...
  def _compute_en_used_hour(self):
    for rec in self:
      ...
      create_date = timezone('UTC').localize(create_date).astimezone(
        timezone(self.env.user.tz or 'UTC')).replace(tzinfo=None)
      create_date = timezone(self.env.user.tz or 'UTC').localize(
        datetime.combine(create_date.date(), time.min)).astimezone(
        timezone('UTC')).replace(tzinfo=None)
      if not self.env['en.resource.planning'].search(
          [('project_id', '=', rec.order_id.project_id.id),
           ('state', '=', 'approved'), ('id', '<', rec.order_id._origin.id)]):
        rec.en_used_hour = en_used_hour
        continue
      wbs_id_lt = self.env['en.wbs'].search(
          [('id', 'in', wbs_ids.ids), ('version_type', '=', 'baseline'),
           ('create_date', '<=', create_date)], limit=1,
          order='create_date desc')
      wbs_id_gt = self.env['en.wbs'].search(
          [('id', 'in', wbs_ids.ids), ('version_type', '=', 'baseline'),
           ('create_date', '>=', create_date)], limit=1,
      ...
      if wbs_id_lt and wbs_id_gt:
        if (wbs_id_gt.create_date - create_date).total_seconds() < (
            create_date - wbs_id_lt.create_date).total_seconds():
          wbs_id = wbs_id_gt
        else:
          wbs_id = wbs_id_lt
      if not wbs_id:
        rec.en_used_hour = en_used_hour
        continue
      min_id = self.env['en.resource.detail'].search(
          [('order_id.project_id', '=', rec.order_id.project_id.id),
           ('employee_id', '=', rec.employee_id.id),
           ('order_id', '=', rec.order_id._origin.id)], limit=1, order='id asc')
      if rec._origin.id != min_id.id:
        rec.en_used_hour = en_used_hour
        continue
      en_task_position = self.env['en.workpackage'].search(
          [('id', 'child_of', wbs_id.workpackage_ids.ids)])
      tasks = self.env['project.task'].search(
          [('en_handler', '=', rec.employee_id.user_id.id),
           ('en_task_position', 'in', en_task_position.ids)])
      effective_hours = sum(tasks.mapped('effective_hours'))
      a = 0
      plannings = self.env['en.resource.planning'].search(
          [('project_id', '=', rec.order_id.project_id.id),
           ('order_line.employee_id', '=', rec.employee_id.id),
           ('id', '<', rec.order_id._origin.id),
           ('state', 'in', ['approved', 'expire'])], limit=1,
          order='create_date desc')
      for p in plannings:
        for l in p.order_line:
          ...
          a += l.en_used_hour
      planning_id = self.env['en.resource.planning'].search(
          [('project_id', '=', rec.order_id.project_id.id),
           ('id', '<', rec.order_id._origin.id),
           ('state', 'in', ['approved', 'expire'])], limit=1,
          order='create_date desc')
      for l in planning_id.order_line:
        if l.employee_id != rec.employee_id: continue
        ltime_start = timezone(self.env.user.tz or 'UTC').localize(
          datetime.combine(l.date_start, time.min)).astimezone(
          timezone('UTC')).replace(tzinfo=None)
        ltime_end = timezone(self.env.user.tz or 'UTC').localize(
          datetime.combine(l.date_end, time.max)).astimezone(
          timezone('UTC')).replace(tzinfo=None)
        ...
        workrange_hours = calendar.get_work_hours_count(datetime_start,
                                                        datetime_end,
                                                        compute_leaves=False)
        range_leave_intervals = employee.list_leaves(datetime_start,
                                                     datetime_end)
  ...
  en_user_id = fields.Many2one(string='User của nhân viên',
                               related='employee_id.user_id')
  order_id = fields.Many2one(string='Kế hoạch nguồn lực',
                             comodel_name='en.resource.planning', required=True,
                             ondelete='cascade', index=True, auto_join=True)
  type_id = fields.Many2one(string='Loại', comodel_name='en.type',
                            compute_sudo=True, compute='_compute_type_id',
  ...
  employee_id = fields.Many2one(string='Tên nhân sự',
                                ...
                                domain="[('en_type_id','=?',type_id), ('id', 'in', employee_ids)]",
                                context={'active_test': False}, index=True)
  en_replacement_staff = fields.Many2one(string='Thay thế cho nhân viên',
                                         comodel_name='hr.employee',
                                         domain="[('en_type_id','=?',type_id)]",
  ...
  @api.depends('order_id.project_id',
               'order_id.project_id.en_resource_project_ids')
  ...
  role_id = fields.Many2one(string='Vai trò', comodel_name='en.role',
                            required=True, domain="[('id', 'in', role_ids)]")
  role_ids = fields.Many2many(string='Vai trò', comodel_name='en.role',
                              compute='_compute_role')
  job_position_id = fields.Many2one(string='Vị trí',
                                    comodel_name='en.job.position',
                                    required=True,
                                    domain="[('id', 'in', job_position_ids)]")
  job_position_ids = fields.Many2many(string='Vị trí',
                                      comodel_name='en.job.position',
                                      compute='_compute_role')
  ...
  def _compute_no_delete_line(self):
    ...
    for rec in self:
      ...
      if (rec.date_start and not rec.date_start >= month_check) or (
          rec.date_end and not rec.date_end >= month_check):
        no_delete_line = True
  ...
  def _onchange_employee_data(self):
    for rec in self:
      if rec.employee_id:
        resource = rec.order_id.project_id.en_resource_project_ids.filtered(
          lambda x: x.employee_id == rec.employee_id and x.state == 'active')
        ...
        rec.job_position_id = resource.en_job_position_ids[
          0].id if resource.en_job_position_ids else False
      ...
      if rec.old_line_id:
        if date_today <= 5:
          if rec.date_start and rec.date_start < month_pre:
            raise exceptions.ValidationError(
              f'Giá trị ngày bắt đầu phải lớn hơn hoặc bằng {month_pre.strftime("%d/%m/%Y")}')
        else:
          if rec.date_start and rec.date_start < month_current:
            raise exceptions.ValidationError(
              f'Giá trị ngày bắt đầu phải lớn hơn hoặc bằng {month_current.strftime("%d/%m/%Y")}')
      ...
      if rec.old_line_id:
        if date_today <= 5:
          if rec.date_end and rec.date_end < month_pre:
            raise exceptions.ValidationError(
              f'Giá trị ngày kết thúc phải lớn hơn hoặc bằng {month_pre.strftime("%d/%m/%Y")}')
        else:
          if rec.date_end and rec.date_end < month_current:
            raise exceptions.ValidationError(
              f'Giá trị ngày kết thúc phải lớn hơn hoặc bằng {month_current.strftime("%d/%m/%Y")}')
  ...
  def check_date_resource(self):
    for rec in self:
      ...
      if rec.date_start and rec.order_id.project_id.date_start and rec.date_start < rec.order_id.project_id.date_start:
        raise exceptions.UserError(
          'Thời gian bắt đầu sử dụng nguồn lực không được nhỏ hơn thời gian bắt đầu dự án!')
      ...
      if rec.date_start and rec.date_end and rec.date_start > rec.date_end:
        raise exceptions.UserError(
          f'Thời gian kết thúc sử dụng nguồn lực {rec.employee_id.display_name} ở {rec.job_position_id.display_name} không thể nhỏ hơn thời gian bắt đầu là {rec.date_start.strftime("%d/%m/%Y")}.')
  ...
  def check_date_resource_1(self):
    for rec in self:
      resource_project = self.order_id.project_id.en_resource_project_ids.filtered(
        lambda
            x: x.employee_id == rec.employee_id and x.date_start and x.date_end)
      in_project = False
      expt_txt = ''
  ...
  def check_workload_borrow(self):
    for rec in self:
      resource_project = self.order_id.project_id.en_resource_project_ids.filtered(
        lambda x: x.employee_id == rec.employee_id and x.is_borrow)
      if resource_project:
        department_resources = self.order_id.project_id.en_department_id.employee_borrow_ids.filtered(
          lambda
              x: x.employee_id == rec.employee_id and x.date_start <= rec.date_start and rec.date_end <= x.date_end)
  ...
  mh = fields.Float(string='🤣', compute_sudo=True, compute='_compute_m_uom',
                    store=True)
  md = fields.Float(string='🤣', compute_sudo=True, compute='_compute_m_uom',
                    store=True)
  mm = fields.Float(string='🤣', compute_sudo=True, compute='_compute_m_uom',
                    store=True)
  ...
  def _compute_m_uom(self):
    ...
    for rec in self:
      ...
      employee = rec.employee_id
      calendar = employee.resource_calendar_id
      for date_step in date_utils.date_range(datetime_from, datetime_to,
                                             relativedelta(months=1)):
        compared_from = max(date_step + relativedelta(day=1),
                            datetime_from).date()
        compared_to = min(date_step + relativedelta(months=1, day=1, days=-1),
                          datetime_to).date()
        comparedtime_from = timezone(self.env.user.tz or 'UTC').localize(
          datetime.combine(compared_from, time.min)).astimezone(
          timezone('UTC')).replace(tzinfo=None)
        comparedtime_to = timezone(self.env.user.tz or 'UTC').localize(
          datetime.combine(compared_to, time.max)).astimezone(
          timezone('UTC')).replace(tzinfo=None)
        ...
        datetime_start = timezone(self.env.user.tz or 'UTC').localize(
          datetime.combine(date_start, time.min)).astimezone(
          timezone('UTC')).replace(tzinfo=None)
        datetime_end = timezone(self.env.user.tz or 'UTC').localize(
          datetime.combine(date_end, time.max)).astimezone(
          timezone('UTC')).replace(tzinfo=None)
        workrange_hours = calendar.get_work_hours_count(datetime_start,
                                                        datetime_end,
                                                        compute_leaves=False)
        ...
        md_ratio = workrange_hours / (
              workmonth_hours / 8) if workmonth_hours else 0
  ...
  en_md = fields.Float(string='MD', compute_sudo=True, readonly=False,
                       compute='_compute_en_m_uom', store=True)
  en_md_migrate = fields.Float(string='MD', compute_sudo=True, compute=False,
    ...
    for rec in self:
      ...
      for date_step in date_utils.date_range(datetime_start, datetime_end,
        ...
        if round(sum(rec.order_id.order_line.filtered(
            ...
            'workload')) + sum(self.env['en.resource.detail'].search(
