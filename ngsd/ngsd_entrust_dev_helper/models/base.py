...
class Base(models.AbstractModel):
    ...
    def open_form_or_tree_view(self, action, form=False, records=False, context={}, action_name=False, target='current'):
        ...
        if 'views' in action and form:
            action['views'] = [(self.env.ref(form).id, 'form')] + [(state, view) for state, view in action['views'] if view != 'form']
        if (not records and context.get('create', self.env[action['res_model']].check_access_rights('create', raise_exception=False))) or len(records) == 1:
            if form:
                action['views'] = [(self.env.ref(form).id, 'form')]
            else:
                action['views'] = [(state, view) for state, view in action['views'] if view == 'form']
            if not action['views']: action['views'] = [(False, 'form')]
            if len(records) == 1: action['res_id'] = records.id
        elif len(records) > 1:
    ...
    def send_notify(self, message, users, subject='', model_description='', access_link=False):
        self.clear_caches()
        view = self.env['ir.ui.view'].browse(self.env['ir.model.data']._xmlid_to_res_id('ngsd_entrust_dev_helper.notify_record_message'))
