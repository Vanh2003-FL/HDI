...
class View(models.Model):
  ...
  def _postprocess_access_rights(self, node):
    """ Compute and set on node access rights based on view type. Specific
        ...
        Updated for Odoo 18 compatibility - removed model parameter. """

    # Call parent with new signature (Odoo 18)
    super(View, self)._postprocess_access_rights(node)

    # For gantt views, we need to get the model from context
    if node.tag in ('gantt'):
      # Try to get model from context or view
      model = None
      if hasattr(self, 'model') and self.model:
        model = self.env[self.model]
      elif hasattr(self, '_name') and self._name:
        model = self.env[self._name]

      if model:
        # testing ACL as real user
        is_base_model = self.env.context.get('base_model_name',
